# AI Companion · 接班总账 v29

更新时间：2026-08-16（Asia/Tokyo）

> 本文件继承并取代 `AI_Companion_接班总账_v28_2026-08-15.md`。判断优先级固定为：用户最新明确决定 > GitHub `main` 实际源码与 Actions > 最新脱敏真机诊断 > 仓库 HANDOFF/任务账 > 历史对话与参考资料。讨论、设计、真机线索不得写成已实现功能。
...
- 默认分支：`main`
- 源码唯一真源：`app/`
- 当前已合并基线为 `v0.33.4+59`，main squash 提交 `c7545cc5e64e8401f76f80fd1241fc187f18cd1f`，schema 21；PR #15 已合并。
- 已合并分支：`codex/v0334-pet-motion-modes-d2-2`；PR #15。产品实现 head 为 `3591ba3635c03381cd84216f9799fcce328ded2b`。
- 当前版本：`v0.33.4+59`；schema 21，不做数据库迁移。
- 本版改动：实现自由/贴边/四种半屏行动模式、轻放/甩动判定、贴边持久吸附、旋转归一化重映射、竖屏底边与横屏左 inset 自适应，以及两套菜单的标题栏圆形关闭和边界裁切。
- Clean Freeze 继续有效：仓库根目录不使用历史 patch/ZIP 作为构建输入；workflow 只验证和构建已提交的 `app/`。
- PR #15 已 squash 合并；main 提交 `c7545cc5e64e8401f76f80fd1241fc187f18cd1f`。
- 产品 Actions run #60（ID `31892199358`）通过完整素材恢复、全部历史 validators、新 D2.2 validator、Kotlin 桌宠单测、Flutter analyze/tests、release APK、native/payload 核验及 artifact 上传。
- 交付 Artifact `9248998863`，名称 `AI-Companion-v0.33.4-59-Pet-Motion-Modes-D2-2-APK`，ZIP digest `sha256:27ab3806966bd063ffa69be3b25cff186b1c1cd2c04a1c80ea7e9c422f1a7174`。
- 交付 APK SHA-256：`0d988be73e4d70e6a2dd23c06def10e2e31f4344a23c0fcebfd0218e55a43e89`；238,606,828 bytes。
