...
| D4 · 单聊表情包表达层（CI PASSED / APK READY / TRUE DEVICE PENDING） | 单包与原子组合包导入、本地随机表达、附件事务、GIF、固定 180dp 宽度及发送后第一人称 caption 已实现；默认频率为“自然”。**尚无明确用户指令的 Agent 发送工具，因此“发个表情包”当前可能文字幻觉，不得误报已完成** | v0.41.45 轻量 smoke 可与开发并行，不硬等自然频率；v0.41.46 先补 `sticker.send`、Agent 自读与真实 Outcome。联网图/相册图在后续独立媒体版本复用同一永久合同 |
| D5 · 媒体 Agent 调用闭环（NEXT） | 先完成明确表情包指令；所有媒体能力必须同时有 Agent registry/self-reader、真实执行、附件事务、失败真值、来源与发送后自我认知 | v0.41.46 与 NSFW 续写身份热修、规则01默认文案清理分提交共用一个 APK；普通联网图/相册图另开下一版本，自主发图以后再接统一仲裁 |
