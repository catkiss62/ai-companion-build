...
| 当前代码 head / tree | v0.41.41 接班断言同步 head `71f88854a05f7719b534edfaff5535332a97e22d` / tree `3e6c7c79df627b74a8831364a71fb5f6f65b04d1`；run 737 通过全部 validators，首次进入 Kotlin/Flutter debug 编译后暴露 const 构造中 Map 取值不是常量表达式。内置日常优先级改为等价字面量 950 待推送 |
...
| 最终 CI | v0.41.41 run [`34004403627`](https://github.com/catkiss62/ai-companion-build/actions/runs/34004403627)（737）已通过全部 validators，并在 Kotlin 步骤的 Flutter debug 编译发现 `const WorldBookPreset` 不能使用 const Map 下标表达式；analyze/test/APK 尚未运行。改为等价值 950 字面量后重跑。上一完整通过仍为 v0.41.40 Actions run [`33991881678`](https://github.com/catkiss62/ai-companion-build/actions/runs/33991881678)（732），603/603 Flutter tests、Release APK、签名与资源载荷全绿 |
