...
| 当前总状态 | +203 为可回退的完整恬豆构建；+204 小酒狐实现提交 `103a671`、成对模型/临时残片加固提交 `b715568` 与 Gemini 中转/思考摘要提交 `1651359` 已完成，当前为 `IMPLEMENTED / LOCAL STATIC PASSED / CI FAILED EARLY / TRUE DEVICE PENDING`。私有小酒狐 APK 已保存到 AI 伴侣未发布 Draft并再次核验 649,663,221 bytes / SHA-256 `af90ae...eb43`。首轮 run `34559566810` 在编译前的 payload unzip 失败：v0.7.6 小酒狐测试 APK含 `benchmark_jiuhu` 与 `libopenjtalk_native.so`，但不含伴侣三语链所需的 `assets/openjtalk/*` 和 `libgenie_frontend.so`；代码、测试和 APK 尚未运行。窄修只从旧已验证 v0.6.4 私有 Draft 恢复这些共用前端运行依赖，小酒狐声学权重/四候选仍只来自 v0.7.6，禁止把恬豆 case 解入生产路径。API 实现固定玩游端点/模型，Gemini 可见聊天请求原生思考摘要并继续显示到现有 reasoning 面板；没有用户 Key，不能伪称真实中转已通过 |

### 3. 当前下一步任务包（新窗口必须完整接住）

| 字段 | 当前内容 |
|---|---|
| 当前下一步 | **修正 +204 私有载荷拆源后重跑 Actions**：小酒狐模型与四候选只从 v0.7.6 Draft APK恢复；`assets/openjtalk/*`、`libopenjtalk_native.so`、`libgenie_frontend.so` 与匹配 `libc++_shared.so` 继续从 +203 已构建验证的 v0.6.4 私有 APK恢复。两个 APK分别按既有固定 SHA 校验，工作流不得从 v0.6.4 解出任何 `assets/benchmark/*`。随后重跑完整源码、Kotlin/Flutter、Release、签名与实包退休语音排除门 |
