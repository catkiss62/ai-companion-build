...
## 当前接班入口（默认只读到“历史工作记录”之前）

> 本入口于 2026-08-31 建立，用来解决“每次对接都把近 770 KB 总账全文和全项目一次性塞进上下文”的问题。它不是删减历史，也不是只看最新版本：下方原有 4,312 行工作记录已逐字保留为可检索档案。日常接班先读本入口；修改旧功能时，再按模块索引定向回读档案、源码、测试、validator、提交与真机证据。

### 1. 接班与减负读取协议

1. **第一次接班默认读取**：本入口、`app/docs/DOCUMENTATION_MAP.md`、当前分支/提交、`app/pubspec.yaml`、`AppDatabase.schemaVersion`、最近一次 CI 和与当前任务直接相关的源码/测试。读到 `## 历史工作记录（原文保留，按需检索）` 即停，不默认继续吞入全文。
2. **修改旧功能时强制扩读**：先在本入口找到模块与历史版本范围，再用功能名、类名、版本号、失败关键词搜索历史档案；随后读取当前源码、对应测试、Python validator、专项文档、相关提交与最新真机结论。找不到、状态冲突或涉及跨域不变量时，扩大检索直到证据闭合。
3. **只有这些任务默认全文审计**：再次重构总账、系统级架构/数据迁移、无法定位来源的跨模块回归、历史状态互相矛盾、或用户明确要求完整审计。全文审计可以在文件侧机械扫描、分段摘要和覆盖核对，不必把全文原样输出到同一轮模型上下文。
4. **旧聊天窗口不是默认数据源**：不需要为了省额度删除同项目旧窗口；日常交接不应把所有旧窗口逐个重读。优先级仍为“用户最新明确决定 > 当前 GitHub 源码及 CI/真机证据 > 最新脱敏诊断 > 本入口 > 历史工作记录 / Git 历史”。需要找某个未落账的决定时才定向检索旧对话。
5. **状态词必须严格使用**：`DESIGNED`、`IMPLEMENTED`、`CI PASSED`、`APK READY`、`TRUE DEVICE PASSED`、`PENDING`、`FROZEN`、`NOT_IMPLEMENTED`、`SUPERSEDED` 含义不可互换。自动化通过不能写成真机通过；曾失败但已被后续版本覆盖的记录不能误报成当前失败。
6. **以后如何更新**：新任务开工前和完成后都写在本入口与历史档案标记之间；先更新当前快照、任务状态和模块索引，再写必要的完整过程。旧记录只追加纠错注释，不静默改写。每次代码批仍记录版本/schema/分支/提交/CI/APK/SHA/真机边界；纯文档批不升版本、不冒充运行变更。
7. **减负不等于漏读**：普通接班获得的是“当前全局状态 + 查找地图”；真正动手时获得的是“该模块完整历史 + 当前实现证据”。禁止只凭顶部摘要直接改旧功能，也禁止每轮无差别加载整个仓库与全部历史。
...
| 当前开发分支 | `agent/v04115-phase2a-self-experience-desire-web`；从 v0.41.14 最终 CI/APK 代码树开出的 Phase 2A 独立批，已完成 CI/APK 并待真机，Phase 1 学习消费与 Phase 2B 回复 bias 仍关闭 |
| 上一运行代码基线 | `agent/v0417-forthright-fiery-personality`，功能 head `58c244a4b08033f403776f1ec31bbece5557506d`；Desire/Moe/主动性状态主干仍沿革自 `agent/v0415-personality-state-diversity` / `494796ef02e369f98e6896bc5acea7185e3c35dd` |
| 当前代码 head / tree | 最终 APK 构建输入 commit `47b29f0a1ff1c638a363b0a3803ef4caf5712521`、tree `82c4633abc3056e845cb4d122601b50e0aa09b65`；它只在首轮功能 tree 上修正新增测试期望并补 run 665 证据，没有改变运行系数。其后仅允许以 `[skip ci]` 总账封存提交前进文档，不得把文档 head 冒充 APK 输入 |
...
| 当前总状态 | 当前可安装测试基线为 `v0.41.15+154 / schema 43`，状态 `CI PASSED / APK READY / TRUE DEVICE PENDING`。首轮 run 665 的单个新增测试期望错误已由 run 666 全量闭合，不是运行逻辑失败。屏幕问题保持冻结。Phase 0+1 观察隔离不变，Phase 2B/3/4 继续关闭；不允许自主截屏、视频、MCP 或学习结果提前影响回复 |
...
| Desire / Thought / Intent / Gate、主动联系 | v0.41.15 已把自然回归改为 baseline-centered、耦合改为相对各轴自身 baseline，并增加 14 天无正文来源事件；Self-Drive 已接可去重候选与真实体验终态；同一熄屏会话不再每个心跳重复增加互动欲；CI/APK 已通过 | 尚待真机自然观察；系数没有整体放大。熄屏仅是 contact window，不是“用户空闲”，夜间按疲劳曲线显著折减 |
...
| 模拟手机、浏览器、私人相册 | 用户消息附件强绑定与相册底座已实现；新存档确认旧网页候选曾“识图摘要与来源图片不一致”。2026-09-02 源码复核又确认：心情图当前取最近 7 **条**记录而不是 7 个自然日；购物车并非 API 随机生成，而是从固定 5 个正常＋5 个搞怪条目中按日稳定选择 2＋2，因此内容重复是结构性现状 | P0 仍先修联网图片同一字节事务与受控保存路由；心情改为固定最近 7 个自然日且同日多条分开；购物车后续用一次有界 API 生成 6 件、近期去重和丰富离线兜底，不能把当前固定目录误报成 DeepSeek 随机性不足 |
| 沉浸房间 / NSFW | v0.39.x 已实现并持续回归；当前页面仍是 `Positioned.fill` 全屏聊天，尚未复用普通聊天的拖动高度 | 成人关系方向、Reality Identity、Session 隔离不可被通用安全模板或女性向材料覆盖；后续仅增加独立 `immersive_panel_fraction` 和同款拖动手柄，不改变房间 prompt、Memory 或 Session |
...
| UI 信息架构、快捷侧栏与文字层级 | 部分五域信息架构已实现；头像/名称左侧快捷面板仍把导航、频率、立绘、背景、TTS、情绪、逐段打字与透明度混在同一长抽屉。全局为暗色主题，部分页面标题和说明都用高亮白色，层级不足 | 本轮只做“优化”而非换肤：标题用高强调色，说明/副文用语义次级色；浅色卡片才用近黑文字，禁止全局把非标题硬改为黑色而导致暗色背景不可读。侧栏按第 21 节拆为干净入口页，并新增只读欲望/萌属性状态页；不得暴露调试动作或隐藏推理 |
...
| P0 · TRUE DEVICE PASSED / PHASE 1 HOLD | 人格学习与成长主框架 Phase 0+1 | `0.41.11+150 / schema 42` 固定六句回放最终为 candidate 2、evidence 4、support 3、contradict 1、semantic review requested/support 各 1；备份逐条确认同义支持归并第一候选、故障偏好与纠正只作用第二候选，没有误绑。Phase 1 真机闭合但继续只观察；Phase 2 尚未实施，须另批设计/授权，不能自动打开回复消费 |
...
| P1 · 观察期插队 / UI-A | 心情日期轴、塔罗一次 3D 入场、沉浸聊天拖动与文字层级 | 只改读取/呈现，不写 Desire、Thought、Memory、候选、联网状态或数据库：心情按最近 7 个自然日固定横轴并分开同日点；每张塔罗首次可见时横向 3D 旋转一周后停止并尊重减少动态效果；沉浸房间使用独立高度设置复用普通聊天拖动手柄；按表面使用语义次级文字色。可在 Phase 2A 等待期开发，但新的 APK 不应在拿到首份 v0.41.15 自然诊断前覆盖安装 |
| P1 · 观察期插队 / UI-B | 快捷侧栏重组与只读“她现在的状态” | 抽屉只保留干净的功能入口，分为内容与模式、她现在的状态、主动联系、聊天画面、语音与情绪、文字演出、全部设置；状态页只显示八项欲望和九项萌属性的当前值/长期基线，不含候选、内部诊断或强制动作。属于较大的导航重构，排在 UI-A 后，必须复用同一设置存储/控件而非复制第二套状态 |
| P1 · 插队但非纯 UI | 购物车 6 件 API 多样生成 | 当前固定 10 选 4 是重复根因。后续每个刷新周期最多一次有界 DeepSeek JSON 生成，输出 6 件且兼有正常与搞怪商品；校验标题唯一、长度、价格和安全边界，以近期标题指纹降重，失败回退到显著扩大的本地目录。它会新增 API/存储生产者，等待 Phase 2A 首轮观察后再实现，不混入 UI-A |
| P1 · 设计完成 / 延后实现 | 总设置分类与自检分层 | 建议按“模型与账号、记忆与成长、主动与感知、语音与表达、设备与数据、诊断与开发”六域重构；先抽共享设置模型和分域保存，再拆页面，避免当前单一 `_save()` 被局部页误覆盖。自检不删除，只把普通用户最有用的快速预检前置，其余按第 21 节说明归档 |
