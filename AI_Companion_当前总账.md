...
## 0S. 2026-08-24 · v0.37.4 音频仲裁、已读演出与双聊天渲染稳定化（IMPLEMENTATION PLANNED / NO APK YET）

> REDMI K80 Ultra 对 v0.37.3+92 的其余19类立绘、动画、自定义舞台、分段气泡、正文安全与聊天主链验收未发现新问题；本节登记本轮新增真机问题和用户已确认的实现方向。按项目约定，这是正式修改前的第一次总账更新；代码、CI、APK 与真机结论尚未产生，不得提前写成完成。

### A. 本轮实施范围

1. **情绪短音效优先，TTS 并行合成、顺序发声**：19类前导情绪信封确定后，每轮助手回复最多触发一次约1秒的短音效；TTS 合成同时开始，不因音效暂停推理或合成。若 TTS 先准备好，只等待当前短音效结束并留极短衔接；若短音效先结束而 TTS 仍在合成，TTS 一旦完成立即播放，不增加等待。停止/取消需同时终止本轮短音效与语音。播放所有权从分段气泡动画移到统一音频调度，避免每个气泡重复触发。
2. **App 已读演出**：修复聊天页初始化通知早于历史 ID 登记，导致每次重新进入都把最后一条旧回复重新执行打字机的问题。当前页面内新生成的回复照常演出；历史已展示回复直接显示；尚未在 App 内展示过的主动消息首次进入仍演出一次，完成展示后持久登记。使用现有 settings 保存展示游标，不升级 schema。
3. **App/悬浮流式对白着色**：解析器支持未闭合的 `「`、`“` 与 ASCII 引号，流式收到开引号后立即按对白色渲染，不必等闭合符。最终闭合文本与历史文本保持原规则。
4. **悬浮聊天文字表现对齐**：动作/神态保留括号并使用白色斜体；`「对白」`、中文双引号与 ASCII 双引号使用与 App 一致的浅红色和常规字重。旧的无引号历史文本不由 UI 猜测改写。悬浮窗继续使用原生轻量单气泡/现有流式路径，不在本批加入背景、立绘、WebP 特效或分段打字机。
5. **reasoning 不再因英文占比整段丢弃**：DeepSeek thinking 继续每轮开启，不增加“显示/不显示”或“思考/不思考”开关。移除主要英文 reasoning 的丢弃过滤，Provider 返回的 reasoning 为空时才不显示；现有“可见思考尽量使用自然中文”契约只做轻量优先级强化，不加入强制逐步中文推理，避免改变思考质量。
...
- 本批不恢复 native 19emo/ONNX，不接 MiniMax TTS，不实施长期 Emotion Appraisal，不开始主动话题与自主搜索。
- 悬浮分段＋逐字演出技术上可行但涉及原生 ListView、滚动、TTS 和中断同步，继续作为后续独立阶段；不能为了合批降低当前稳定性。

### C. 完成判据

只有音频单轮所有权与先音效后人声的时序测试、慢/快 TTS 两种竞态测试、Stop 测试、历史/主动消息展示游标测试、App/悬浮未闭合引号与颜色/斜体测试、reasoning 保留测试、全部历史 validators、Kotlin/Flutter 测试、Release APK、签名/载荷/checksum 与 Draft Release 上传全部通过，并进行第二次总账回填后，才可标记源码与 CI 完成；REDMI K80 Ultra 仍需真机检查短音效—TTS 衔接、重进聊天、主动消息首次演出和悬浮文字颜色。
