...
- 用户中途校准修正（待 CI/真机验证）：Photoshop RGB 输入色阶指定 `0 / 0.86 / 230`、输出 `0 / 255`，替换原单参数“提亮曲线”。App 内新增黑场、中间调和白场控制，默认采用该数值；逐像素对 RGB 应用输入色阶并保留原 alpha，饱和度仍独立可调。旧版保存的 gamma 不强制沿用到新色阶，以避免混用两种不同定义；缩放、位置和饱和度存档保持读取。此项当前仅本地实现，尚未构建或真机确认。
