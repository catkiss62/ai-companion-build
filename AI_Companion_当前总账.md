...
## 0H. 2026-08-23 v0.36.3 真机回归分析与 LingChat 本地 TTS 评估（ANALYSIS ONLY / NO CODE CHANGE）

- 本轮仅检查、取证与更新总账，没有修改功能源码、版本号、schema 或构建。用户真机确认：v0.36.3 的桌宠 / HyperOS Game Turbo Cover 问题已恢复；该项从“待确认”提升为真机通过。
- “5分钟后找我”探针并非按钮未执行：脱敏诊断记录为本地约 04:13:08 安排、04:18:08 到期、04:21:53 实际触发，延迟 225,008 ms（约 3 分 45 秒）；原生状态为 completed，notificationPosted=true，应用解析成功为 social，无取消、拒绝或发布失败。因此需要拆成两个问题：① `setAndAllowWhileIdle` 的近似闹钟导致到期不精确；② Android 已接收高优先级通知，但 HyperOS 只显示状态栏、没有像 QQ 一样的悬浮横幅和提示音。实际 AI 主动消息也已成功进入会话，说明主动联系主链路不是本次主要故障。
- 当前建议不再盲修“5分钟按钮”。它仍可保留为诊断探针，但后续应把文案改为“约5–10分钟”或明确“系统可能延迟”；只有确需秒级测试时才评估精确闹钟权限，不为普通主动消息引入闹钟/来电级全屏权限。
- 通知声音与横幅暂列同一真机配置/频道诊断：v0.36.3 报告显示消息频道 `companion_messages_chime_v1`、importance=HIGH(4)、lastSound=chime、lastPosted=true。Android 8+ 频道创建后，横幅/声音等行为最终由用户和系统控制，App 重建同名频道不能覆盖用户设置；先检查 HyperOS 的应用通知、对应消息频道、悬浮/横幅、声音、勿扰和通知音量。后续源码若处理，先补“实际频道 importance/sound/vibration + 打开该频道系统设置”的可见诊断；若新频道 ID 下自带 OGG 仍失声，再把“系统默认音”作为低风险兜底，不先换音频文件赌厂商兼容性。
...
- LingChat 当前 main（检查时 HEAD `eae0d667413e490c3653488d43ce9b4464e07fda`）的“本地 TTS”可以作为技术来源，但不能直接复制进现有 Flutter/Kotlin App。它是 Tauri/Rust 进程内 Style-Bert-VITS2：`LocalTtsEngine` 通过 `ort` + `sbv2_core` 加载日语 DeBERTa ONNX、tokenizer、声音 ONNX 和 style vectors，再输出 WAV；不是可直接调用的 HTTP 服务或 Android AAR。项目准备了 Android/aarch64 构建路径，但这不等于该引擎已经在本项目内完成真机验证。
- 体积与适用范围明显：默认模型清单约为 DeBERTa 278 MB + tokenizer 2.1 MB + Ling-v2 声音 249 MB，合计约 529 MB（未计运行内存/缓存）；上游当前仅支持 JP-Extra 日语。对已有 Meju A2 / MNN 本地 TTS 的本项目，更适合作为“可选日语 SBV2 引擎/用户自导入声音包”的后续实验，而不是直接替换现有主引擎。
- 许可边界：LingChat 仓库整体为 AGPL-3.0，直接复制其 wrapper/registry/engine 代码会引入 AGPL 分发义务；其依赖的上游 `sbv2_core` 标注 MIT，更稳妥的路线是只参考接口与架构，针对 permissive upstream 独立实现干净适配层。Ling-v2 模型在当前 registry 中只有下载地址，未发现足以确认再分发权的模型许可，禁止在许可核实前把模型随 APK/Release 分发。
- 若后续批准实施，先做隔离 POC 而不是接入正式分支：arm64 Android Rust/NDK 最小构建 → 加载用户侧下载模型 → 合成一条短日语 → 记录冷启动、峰值 RAM、CPU/耗时、后台驻留和电量 → 再决定是否提供可选引擎。参考入口：[LingChat 本地 TTS 文档](https://github.com/SlimeBoyOwO/LingChat/blob/main/docs/local-tts-api.md)、[当前引擎实现](https://github.com/SlimeBoyOwO/LingChat/blob/main/src-tauri/src/ai_service/tts/local/engine.rs)、[模型注册表](https://github.com/SlimeBoyOwO/LingChat/blob/main/src-tauri/src/ai_service/tts/local/registry.rs)、[Tauri Cargo 依赖](https://github.com/SlimeBoyOwO/LingChat/blob/main/src-tauri/Cargo.toml)、[AGPL LICENSE](https://github.com/SlimeBoyOwO/LingChat/blob/main/LICENSE)。
