...
0. 当前源码候选为 `v0.35.4+79 Prompt Format & Chat UI`：新版规则 06 已逐字替换且 ID/路由标记完整；同一动作与神态格式按指定标题加入实际属于 02 的【叙事克制】之后。App/悬浮聊天点开默认回到底部，悬浮思考入口改为横条；试穿移到 NSFW 左侧，NSFW 缩至 24dp 并使用主题浅紫。聊天思考开关与持久设置已删除，聊天/图片/恢复/主动消息固定开启思考。schema 仍为 26，本地静态校验通过，CI/APK/真机尚待验证。已构建基线仍为 v0.35.3+78。
