...
class _ChatPageState extends State<ChatPage> with WidgetsBindingObserver {
  ...
  Future<void> _chooseImageSource() async {
    ...
    final guardStarted = DateTime.now();
    var pickerOutcomeRecorded = false;
    var guardOutcomeRecorded = false;
    try {
      await AttachmentPipelineTelemetry.record(
        AppDatabase.instance,
        stage: 'overlay_guard',
      ...
      try {
        ...
        await AttachmentPipelineTelemetry.record(
          AppDatabase.instance,
          stage: 'picker',
          outcome: image == null ? 'cancelled' : 'completed',
          source: sourceKey,
          duration: DateTime.now().difference(pickerStarted),
        );
        pickerOutcomeRecorded = true;
      } catch (error) {
        await AttachmentPipelineTelemetry.record(
          AppDatabase.instance,
          stage: 'picker',
        ...
        try {
          ...
          await AttachmentPipelineTelemetry.record(
            ...
            outcome: 'completed',
            source: sourceKey,
            duration: DateTime.now().difference(guardStarted),
          );
          guardOutcomeRecorded = true;
        } catch (error) {
          await AttachmentPipelineTelemetry.record(
            AppDatabase.instance,
            stage: 'overlay_guard',
  ...
  Future<void> _prepareAndConfirmImage(
    ...
    int? selectedByteSize;
    try {
      selectedByteSize = await image.length();
    } catch (_) {
      // The prepare stage will report a categorized failure if the file is
      // actually unreadable; metadata lookup itself stays best-effort.
    }
    ...
    try {
      ...
      await AttachmentPipelineTelemetry.record(
        ...
        outcome: 'completed',
        source: source,
        duration: DateTime.now().difference(prepareStarted),
                    ...
                    if (item.isInterruption)
                      ...
                      _MessageBubble(
                        ...
                        typewriterPlaybackReady:
                            GenerationPresentationPolicy
                                .typewriterPlaybackReady(
                          animateRequested: _typewriterEnabled &&
                              item.message!.id == latestAssistantId &&
                              item.message!.id == _animatedMessageId,
...
class _MessageBubble extends StatelessWidget {
  const _MessageBubble({
    ...
    required this.typewriterPlaybackReady,
  ...
  final bool typewriterPlaybackReady;
...
class _SingleBubbleTypewriterTextState
  ...
  void initState() {
    super.initState();
    if (widget.playbackReady) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _start());
    }
  }

  @override
  void didUpdateWidget(covariant _SingleBubbleTypewriterText oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (!oldWidget.playbackReady && widget.playbackReady && !_started) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _start());
    }
  }
...
class _AssistantSegmentSequenceState
  ...
  void initState() {
    super.initState();
    if (widget.animate && widget.playbackReady && widget.chunks.isNotEmpty) {
  ...
  void didUpdateWidget(covariant _AssistantSegmentSequence oldWidget) {
    ...
    if (oldWidget.animate && !widget.animate) {
      ...
      _currentCharacters = 0;
        ...
        !oldWidget.playbackReady &&
        widget.playbackReady &&
        !_started &&
  ...
  Widget build(BuildContext context) {
    final visibleCount = widget.animate
        ? (widget.playbackReady
            ? (_completedChunks +
                (_completedChunks < widget.chunks.length ? 1 : 0))
            : 0)
