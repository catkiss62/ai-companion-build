...
这些变化只属于虚构场景，不会写入永久 AI Self、现实 Memory、关系成长或普通主动来源；Session 结束后自然卸下临时外皮。真实工具 Outcome、用户控制权和隐私边界始终不被虚构设定覆盖。''';
  ...
  Future<PromptBuildResult> buildChatPrompt({
    ...
    final roleplayContinuity = worldBookContext.hasRoleplay && session != null
        ? WorldBookHistoryPolicy.continuityBeforeRetainedTurns(
            continuityNote: session.continuityNote,
            retainedHistory: promptRecent,
            activeRoleplaySessionId: worldBookContext.roleplaySessionId,
          )
        : '';
    ...
    final messages = <Map<String, Object?>>[
      ...
      if (roleplayContinuity.isNotEmpty)
        {
          'role': 'system',
          'content': '''【当前角色扮演 Session · 早先剧情尾部】
以下只包含尚未由后续原始消息再次提供的较早剧情，不是现实记忆或 AI 本体事实。若与当前用户消息冲突，以当前消息为准。
$roleplayContinuity''',
        },
...
当前版本从真实用户原话中整理可撤销的偏好/关系许可候选，并经过重复支持、反证与本地裁决改变成熟度。只有 ordinary 语境中已 established、支持充分且无反证的候选，才可能作为最多两条低权重倾向进入普通或主动回复；不要求每轮表现，当前用户纠正和 AI 自己此刻的判断优先。人格候选不会直接写入 AI Self、Desire、Moe、Thought 或 Drive，沉浸场景也不消费这层。另有独立的 AI 自身兴趣证据：只有跨日期成立、仍新鲜且未停用的成熟兴趣，才可在 Phase 3C 的独立预算与冷却内轻量影响自主联网选题或主动话题；它不能绕过原有 Desire、联网和投递 Gate，也不能变成固定台词或永久人格标签。
