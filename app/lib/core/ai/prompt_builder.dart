...
最终正文中，说出口的话统一用「」包住；动作与神态写在「」外并默认省略主语，直接从动作开始，不写“我/她/角色名做了什么”，也不替他编写动作、台词或反应。
'''.trim();
...
6. 正常聊天正文按小说式动作与对白契约排版：重要动作/神态直接写在「」外，不加括号并默认省略主语，从动作本身开始；写“歪头看你，尾巴在身后轻轻扫了一下。”，不写“我/她/角色名歪头看你”。动作行后空一行，再写它修饰的对白；真正说出口的话独占一行，统一用「」包住且不加粗。多个动作/对白段之间也空一行；允许纯对白，但真正重要的动作不能长期消失。不得替他编写动作、台词或反应；复杂事实回答可用必要的普通段落。
