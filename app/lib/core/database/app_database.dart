...
class AppDatabase {
  ...
  Future<void> ensureReady() async {
    ...
    if (MoeExpressionDefaultPolicy.shouldApply(
      moeExpressionDefaultMigration,
    )) {
      // v0.41.25/26 intentionally disabled this optional prompt layer while
      // establishing the minimal-persona baseline. That observation is now
      // complete: enable it once in the explicit "obvious" mode requested for
      // the next comparison. The marker ensures a later user-off choice is
      // never overwritten by normal startup.
      await setSetting(
        'moe_expression_enabled',
        MoeExpressionDefaultPolicy.enabledValue,
      );
      await db.update(
        'moe_config',
        {
          'expression_mode': MoeExpressionDefaultPolicy.expressionMode,
          'updated_at': DateTime.now().millisecondsSinceEpoch,
        },
        where: 'id = 1',
      );
      await setSetting(MoeExpressionDefaultPolicy.markerKey, '1');
    }
  ...
  Future<List<String>> recentDeliveredProactiveSourceTypes({
    ...
    final rows = await db.query(
      ...
      where: "status = 'completed' AND behavior_kind IN "
          "('proactive_message','public_web_share','game_share') "
          'AND started_at >= ?',
      whereArgs: [
  ...
  Future<Map<String, Object?>> autonomousBehaviorDiagnosticStats({
    ...
    final behaviorCounts = await grouped('behavior_kind');
    final sourceCounts = await grouped('source_type');
    final statusCounts = await grouped('status');
    final eventCount = (row['total'] as num?)?.toInt() ?? 0;
    MapEntry<String, int>? dominant(Map<String, int> values) {
    ...
    final dominantBehavior = dominant(behaviorCounts);
    final dominantSource = dominant(sourceCounts);
    final nonproductiveCount = (statusCounts['wait'] ?? 0) +
        (statusCounts['blocked'] ?? 0) +
        (statusCounts['failed'] ?? 0);
    final alerts = <String>[];
    if (eventCount >= 8 &&
        dominantBehavior != null &&
        dominantBehavior.value / eventCount >= 0.60) {
      alerts.add('behavior_dominance:${dominantBehavior.key}');
    }
    if (eventCount >= 8 &&
        dominantSource != null &&
        dominantSource.value / eventCount >= 0.60) {
      alerts.add('source_dominance:${dominantSource.key}');
    }
    if (nonproductiveCount >= 4) {
      alerts.add('nonproductive_winners');
    }
    return <String, Object?>{
      ...
      'nonproductiveWinnerCount': nonproductiveCount,
      'dominanceAlerts': alerts,
  ...
  Future<Map<String, Object?>> companionAlbumDiagnosticStats() async {
    ...
    return {
      ...
      'imageBinding': {
        ...
        'autonomousWebMetadataUsedAsVisionCaption': true,
        'autonomousWebMetadataTrust': 'bounded_untrusted_context_v04133',
