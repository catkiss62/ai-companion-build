...
class SqliteMoeRepository implements MoeRepository {
  ...
  Future<MoeStateSnapshot> loadState() async {
    ...
    try {
      final db = await _databaseProvider();
      final configRows = await db.query('moe_config', where: 'id = 1', limit: 1);
      final config = configRows.isEmpty ? const <String, Object?>{} : configRows.first;
      final mode = moeExpressionModeFromKey(config['expression_mode']?.toString());
      if (config['enabled'] == 0) {
        return MoeStateSnapshot.neutral(now: now, expressionMode: mode);
      }

      final axisRows = await db.query('moe_axis_state');
      if (axisRows.isEmpty) {
      ...
      for (final row in axisRows) {
        ...
        baselines[axis] = clampMoeValue((row['baseline'] as num?) ?? axis.defaultBaseline);
        current[axis] = clampMoeValue((row['current_value'] as num?) ?? baselines[axis]!);
        updatedAt = mathMax(updatedAt, (row['updated_at'] as num?)?.toInt() ?? 0);
        policyVersion = (row['policy_version'] as num?)?.toInt() ?? policyVersion;
      }

      final recipeRows = await db.query('moe_recipe_state');
    ...
    final inserted = await db.insert(
      'moe_events',
      {
        ...
        'pulses_json': jsonEncode({
          for (final entry in event.axisPulses.entries) entry.key.key: entry.value,
        }),
