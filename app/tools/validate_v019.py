...
def check_version_schema() -> None:
    ...
    if not re.search(r'^version:\s*0\.19\.0\+19\s*$', pub, re.M): fail('pubspec version != 0.19.0+19')
    db = (ROOT/'lib/core/database/app_database.dart').read_text(encoding='utf-8')
    if 'static const int schemaVersion = 13;' not in db: fail('schema != 13')
    for token in (
        ...
        if token not in db: fail(f'v13 DB token missing: {token}')
...
def compare_v018_freeze(baseline_zip: Path | None) -> int:
    ...
    allowed_changed = {
        ...
        'lib/features/reference/reference_library_page.dart',
    ...
    with zipfile.ZipFile(baseline_zip) as zf:
        ...
        if not roots:
            fail('cannot infer v0.18 baseline root prefix')
        prefix = roots[0]
        checked = 0
        for name in file_names:
            ...
            if not current.exists():
                fail(f'unexpected v0.18 file deletion: {rel}')
            if hashlib.sha256(current.read_bytes()).digest() != hashlib.sha256(zf.read(name)).digest():
                fail(f'unexpected change outside v0.19 allowlist: {rel}')
            checked += 1
        return checked
...
def main() -> int:
    ...
    checks = [
        ...
        ('Version/schema v13', check_version_schema),
    ...
    for name, fn in checks:
        fn(); print(f'[OK] {name}')
    frozen = compare_v018_freeze(args.baseline_zip)
    if args.baseline_zip is not None:
        print(f'[OK] v0.18 frozen files byte-identical outside allowlist: {frozen}')
    count = compare_tts(args.baseline_zip)
    print(f'[OK] TTS critical files unchanged: {count}')
    print('v0.19 Reference Library / surface consistency validation passed.')
    return 0
