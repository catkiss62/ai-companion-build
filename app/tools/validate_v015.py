...
def check_version_schema() -> None:
    ...
    if not re.search(r'^version:\s*0\.15\.0\+15\s*$', pub, re.M):
        fail('pubspec version is not 0.15.0+15')
    db = (ROOT/'lib/core/database/app_database.dart').read_text(encoding='utf-8')
    if 'static const int schemaVersion = 12;' not in db:
        fail('v0.15 DB schema must be 12')
...
def compare_tts(baseline_zip: Path | None) -> tuple[int, list[str], int]:
    ...
    with zipfile.ZipFile(baseline_zip) as zf:
        ...
        for p in paths:
            ...
            baseline_name = 'ai_companion_v0_14/' + rel
...
def main() -> int:
    ...
    print('v0.15 async-worker ownership/lifecycle validation passed.')
    return 0
