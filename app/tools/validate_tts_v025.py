...
def check_wiring() -> None:
    ...
    expected = {
        ...
        'engine': ['fun verifyArtifacts()', 'speechGeneration', 'beginAudioStream', 'enqueueAudio', 'finishAudioStream'],
        'bridge': ['"verifyArtifacts"', 'tts_verify_failed', '"generate"', '"beginAudioStream"', '"enqueueAudio"', '"finishAudioStream"', 'generationWorker', 'playbackWorker'],
        'provider': ["invokeMapMethod<Object?, Object?>('verifyArtifacts')", "invokeMethod<Uint8List>('generate'", "invokeMethod<void>('beginAudioStream'", "invokeMethod<void>('enqueueAudio'", "invokeMethod<void>('finishAudioStream'"],
        'service': ['implements TtsQueueService', 'Future<TtsStatus> verifyArtifacts()', 'generatePrepared', 'beginPlayback', 'enqueuePlayback', 'finishPlayback', 'Future<void> _recordError'],
        'queue': ['final TtsQueueService service', 'waitUntilIdle()', 'generation-ahead', 'service.enqueuePlayback(audio)'],
