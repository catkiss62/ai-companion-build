...
def check_version_schema() -> None:
    ...
    if not re.search(r'^version:\s*0\.18\.0\+18\s*$', pub, re.M): fail('pubspec version != 0.18.0+18')
...
def compare_v017_freeze(baseline_zip: Path | None) -> int:
    ...
    with zipfile.ZipFile(baseline_zip) as zf:
        ...
        if not roots:
            fail('cannot infer v0.17 baseline root prefix')
        prefix = roots[0]
        checked = 0
        for name in file_names:
            ...
            if not current.exists():
                fail(f'unexpected v0.17 file deletion: {rel}')
            if hashlib.sha256(current.read_bytes()).digest() != hashlib.sha256(zf.read(name)).digest():
                fail(f'unexpected change outside v0.18 allowlist: {rel}')
            checked += 1
        return checked
...
def main() -> int:
    ...
    for name, fn in checks:
        fn(); print(f'[OK] {name}')
    frozen = compare_v017_freeze(args.baseline_zip)
    if args.baseline_zip is not None:
        print(f'[OK] v0.17 frozen files byte-identical outside allowlist: {frozen}')
    count = compare_tts(args.baseline_zip)
    print(f'[OK] TTS critical files unchanged: {count}')
    print('v0.18 companion home validation passed.')
    return 0
