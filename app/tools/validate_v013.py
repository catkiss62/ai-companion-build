...
def check_version_schema() -> None:
    ...
    if not re.search(r'^version:\s*0\.13\.0\+13\s*$', pub, re.M):
        fail('pubspec version is not 0.13.0+13')
    db = (ROOT/'lib/core/database/app_database.dart').read_text(encoding='utf-8')
    if 'static const int schemaVersion = 10;' not in db:
        fail('v0.13 unexpectedly changed DB schema')
...
def compare_tts(baseline_zip: Path | None) -> tuple[int, list[str], int]:
    ...
    with zipfile.ZipFile(baseline_zip) as zf:
        ...
        for p in paths:
            ...
            baseline_name = 'ai_companion_v0_12/' + rel
...
def main() -> int:
    ...
    print('v0.13 true-overlay/lifecycle validation passed.')
    return 0
