# AI Companion · 接班总账 v28

更新时间：2026-08-15（Asia/Tokyo）

> 本文件继承并取代 `AI_Companion_接班总账_v27_2026-08-15.md`。判断优先级固定为：用户最新明确决定 > GitHub `main` 实际源码与 Actions > 最新脱敏真机诊断 > 仓库 HANDOFF/任务账 > 历史对话与参考资料。讨论、设计、真机线索不得写成已实现功能。
...
- D2.2 已锁定行动模式设计但尚未进源码：默认自由行动、四边贴边/底边地面、上/下/左/右半屏自由；轻放在原地播短 `FALLING → LANDING` 而不改变坐标，甩动才保留速度并落到活动区域地面；区域与方向必须随横竖屏重算。
...
- 默认分支：`main`
- 源码唯一真源：`app/`
- 当前已合并基线为 `v0.33.3+58`，main squash 提交 `f06a92d89e58cbf3d93aed5aa884d087c0097913`，schema 21；PR #14 已合并。
- 已合并分支：`codex/v0333-overlay-menus-unread-reliability`；PR #14。产品实现 head 为 `3e9a4caa034367d5265f2064c9e10d8e6a9cbdb4`。
- 当前版本：`v0.33.3+58`；schema 21，不做数据库迁移。
- 本版改动：在 D2 真实桌宠基础上补悬浮球双击菜单/切桌宠/左侧缩进、桌宠可滚动菜单/最近边贴靠、回复提交后未读、空中菜单关闭后继续下落，以及物理全屏边界自动适配。
- Clean Freeze 继续有效：仓库根目录不使用历史 patch/ZIP 作为构建输入；workflow 只验证和构建已提交的 `app/`。
- PR #14 已 squash 合并；main 提交 `f06a92d89e58cbf3d93aed5aa884d087c0097913`。
- run #57 首次 Kotlin 编译准确抓到两处错误的 `ScrollView.LayoutParams`，随后改用继承容器的 `FrameLayout.LayoutParams`；最终产品 run #59 全绿。
- 产品 Actions run #59（ID `31888057510`）通过完整素材恢复、全部 validators、Kotlin 桌宠单测、Flutter analyze/tests、release APK、native/payload 核验及 artifact 上传。
- 交付 Artifact `9247928432`，名称 `AI-Companion-v0.33.3-58-Overlay-UX-D2-1-APK`，ZIP digest `sha256:6d6c866e932161cde1e489098c8d5717f88e75ecb4c6bcbde19a6733c17dc0a5`。
- 交付 APK SHA-256：`2ecc4d6222d85fe2dfee1f6ba8b56f01591d567f5f67e14df880a45588dad484`；238,604,052 bytes。
...
| P2 | IMPLEMENTED/REAL-DEVICE-VERIFY | Android 系统桌宠 D2/D2.1 | 完整素材与 18 动作播放器进入真实 Overlay；二选一入口、双菜单、缩进、未读与全屏边界已过自动验证，待 v0.33.3 真机 |
| P2 | NEXT/DESIGN LOCKED | Android 桌宠 D2.2 行动模式 | 自由、四边贴边、四种半屏；轻放原地短下落，甩动落到区域地面；横竖屏重算 |
...
### 18.4 D2.2 行动模式（DESIGN LOCKED / NOT IMPLEMENTED）
