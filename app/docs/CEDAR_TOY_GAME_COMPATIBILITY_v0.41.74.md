# Cedar Toy 游戏兼容矩阵 · 复核至 v0.41.77+221
...
2026-09-15 重新读取了下表 20 个公开仓库的玩家入口、MCP 工具签名或 AI play guide；9 个 Cedar 站内项目按实时 `get_guide` 合同覆盖。源码审计只用于确认接入边界，不会进入伴侣运行时 Prompt。

公开游戏可归入 `cmd` 单入口、建局参数、严格合法动作、惰性时间、共享网页、多人房间/长轮询、多工具旅程七类，但 APK 不按这七类编写游戏步骤。聚合层已经统一提供 `list_games → get_guide → play(game, action, params)`；伴侣的公共循环固定为：

1. 从实时目录发现目标，允许“陪我下五子棋”这类自然意图由模型匹配目录，而非硬编码游戏名。
2. 完整读取玩家指南和实时 `play` schema；过滤仓库链接、源码、人类攻略、谜底与隐藏状态。
3. Agent 用原生函数调用明确选择 `act / invite_user / await_user / await_remote / complete`；`act` 携带指南内 action 与参数。
4. 执行一次真实 MCP 动作，优先信任结构化 actor、合法动作、续接指令和结束态；自由文本 Outcome 才由同一 DeepSeek 分类器补足。
5. 只有真实服务状态支持等待/用户参与/结束时才停。仅仅读完指南不能进入“等待游戏”；模型流截断或解析失败也不能伪装成等待。
6. 前台聊天与后台时钟共享同一停止语义；后台一次只提交一个有副作用动作，随后依据真实 Outcome 再规划，避免重放不确定写入。
