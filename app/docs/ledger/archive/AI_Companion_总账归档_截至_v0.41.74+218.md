...
> **Cedar MCP 信任优先永久合同（所有后续游戏厅任务强制）**：Cedar Toy 是面向大量人机恋用户、经过长期维护并自带防沉迷/合法动作/轮次/房间/存档约束的成熟 MCP，不按“未知个人分享服务”预设为不可信或先行叠加大量本地防御。对其 `tools/list` schema、`get_guide` 完整指南、结构化 Outcome、`next_actor / next_call / revision / legal_actions / resume_after`、成功/终局和服务端节奏约束默认直接信任并执行，不得再让本地模型二次否定、改写或统一施加五分钟退避；只有真实证据出现后才增加针对已证实问题的最小修复。伴侣侧只保留与本项目直接相关的边界：凭据/隐私不泄漏、同一动作不并发、不可逆外部事务遵守用户授权、MCP 普通文本不能覆盖系统/人格规则、真实 Outcome 与可见声称一致，以及游戏经历如何进入 Thought/分享。不得把这些边界扩张成对 Cedar 选择、调用、返回或合法操作的普遍阻断。
...
| 当前开发分支 | `agent/v04174-cedar-game-hall-protocol`；版本 `v0.41.74+218 / schema 61 / Snapshot protocol 6`，当前状态 `CI PASSED / APK READY / TRUE DEVICE PENDING`。本批以 2026-09-14 新诊断、备份、Cedar 实时 `tools/list`/`account`/`duel` 指南及 20 个公开小游戏仓库为证据，不再按双弈单点补丁：统一接入平台公共动作、结构化回合与只读轮询边界、局部暂停/恢复、近期游戏建议、中文目录/会话面板和房间 DeepSeek 单通道；不改普通聊天/沉浸房间 Gemini、数据库 schema、Snapshot protocol、TTS 或全工具展示 |
| 上一运行代码基线 | `agent/v0417-forthright-fiery-personality`，功能 head `58c244a4b08033f403776f1ec31bbece5557506d`；Desire/Moe/主动性状态主干仍沿革自 `agent/v0415-personality-state-diversity` / `494796ef02e369f98e6896bc5acea7185e3c35dd` |
| 有效构建 head / tree | +218 Actions build head `3163aa6cfac80a14414baa8488950557f26a4245` / tree `7c6f2c7cd8ff5ff2f00678468df7cf6133f5a80e`，远端 tree 与本地构建源码逐字一致。公开提交不含用户附件、诊断、备份、密钥、模型权重、参考音频、声学张量或 APK Artifact |
| App / 数据库 | 最新已构建测试 APK 为 `0.41.74+218 / schema 61 / Snapshot protocol 6`。本批未升级 SQLite schema 或 Snapshot protocol，旧备份兼容边界不变 |
| 最终 CI | +218 run [`34834400059`](https://github.com/catkiss62/ai-companion-build/actions/runs/34834400059)（872）完整成功：总账与变更范围、私有四音色/417 桌宠/LingChat/塔罗恢复、98 项源码/历史 validator、Kotlin/JVM、Flutter Analyze、`789/789` Flutter tests、arm64 Release、固定签名、Genie/Jiuhu/OpenJTalk 实包与 ORT byte identity、四音色每条六参考输入、checksum、Artifact 与 Draft 上传全部通过 |
| 测试 APK | `AI-Companion-v0.41.74-218-Cedar-Game-Hall-Protocol-APK.apk` |
| APK SHA-256 | `3a6cb3a64d0e0374799165fe4e23d03e5042c133d5dac7efcf8823b4a3ec2d86`；CI checksum 与 Draft asset一致，固定测试签名证书 SHA-256 仍为 `305eb3d80983b963c64818ddf1ad561f279de6d47b3ed2c781ada448c7c25148` |
| Artifact / Release | Artifact [`10343866886`](https://github.com/catkiss62/ai-companion-build/actions/runs/34834400059/artifacts/10343866886)，ZIP 537,956,891 bytes / digest `60670b868d2cf4f6affde63928677c64a24ee9c14babc3d3517357c4e6dbd47b`；[Draft Release](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-3f23ace4424267936d8f) 未发布，含 APK、`.sha256` 与 CI monitor，`main` 未合并 |
| `main` | 仍停在 v0.38.5 旧基线，未合并 v0.41.x；**不得从 `main` 误判当前项目或作为后续开发基线** |
| 当前总状态 | +204 小酒狐语音链 `TRUE DEVICE PASSED`；+206 双通道回复主要链 `TRUE DEVICE PARTIAL`；+208 设置精确回退为 `CI PASSED / APK READY / TRUE DEVICE PASSED`。+209、+210 均为 `CI PASSED / APK READY / TRUE DEVICE PENDING`。+211 Cedar 基础链为 `TRUE DEVICE PARTIAL`；+213 为 `TRUE DEVICE PARTIAL · CO-PLAY LOOP BUG FOUND`；+214 为 `TRUE DEVICE PARTIAL · REALTIME/ROOM-CHAT/FINALIZATION BUGS FOUND`；+215 为 `TRUE DEVICE PARTIAL`。+216 为 `CI PASSED / APK READY / TRUE DEVICE PARTIAL`；+217 为 `CI PASSED / APK READY / TRUE DEVICE PARTIAL`；+218 已 `CI PASSED / APK READY / TRUE DEVICE PENDING`，等待防沉迷自重置、双弈回合推进/暂离、中文面板和 DeepSeek 房间对白真机验收 |

### 3. 当前下一步任务包（新窗口必须完整接住）

| 字段 | 当前内容 |
|---|---|
| 当前下一步 | **v0.41.74+218 Cedar 游戏厅真机验收（CI PASSED / APK READY）**：覆盖安装后先在 Cedar 官网开启“允许小机自行重置”，用接近/达到防沉迷阈值的双弈或任意游戏确认她能主动调用 `rest`，且服务端关闭开关时真实拒绝；双弈在人类落子后确认 `your_turn:true` 会转入她的合法落子而非反复 `state`。再说“我暂时要忙，你先玩别的”确认只暂停/释放活动位、不远端弃权，随后从可展开中文会话面板恢复；检查长中文名、省略布局、活动主色边框/查看底色。房间消息确认只走 DeepSeek、失败提示不跨游戏显示；普通聊天/沉浸 Gemini 与四音色只做基本回归。全工具调用动作展示继续作为下一独立任务，不与本次真机结论混写 |
...
| 当前证据 | 新备份中 fishing 与 duel 是独立 session，说明防沉迷不是存档串线；Cedar 官网把绑定小机的连续游玩轮数作为账号级设置，跨全部游戏共享。实时 `tools/list` 明确把 `rest / announcements / vote` 列为跨游戏公共 action，而 duel 指南不重复写 `rest`；+217 执行器只接受“目标游戏指南中出现的 action”，故她的合法自重置在出站前被 APK 拦截。duel revision 9 的 20 余次真实 `state` 都返回 `your_turn:true`，现有 resolver 未识别该布尔字段，持续保留 long-poll，最终 move 才遇 MCP HTTP 失败。备份还记录游戏房间 Gemini 为稳定 HTTP 400 后 DeepSeek 兜底，且提示使用全局 setting 导致任意游戏 session 都显示 |
...
| 实现边界 | 产品入口仍保留独立「Cedar Toy 游戏厅」与通用 MCP transport。稳态单人循环优先直接消费结构化 Outcome：明确 actor/terminal/next_call 不再交给模型二次分类；只有缺少结构化字段时才用轻量 DeepSeek 兜底。notable 判定不得强迫每步重复完整指南；休闲写 Thought，快速/观战通过现有主动对话生成与落库通道直接排队，不另建第二聊天系统。游戏房间短对白是 session 内部动作注释，固定走 DeepSeek；普通聊天与沉浸房间最终可见回复继续遵守既有 Gemini 双通道合同 |
| 完成判据 | 自动化至少按协议类型覆盖：简单 `cmd` 单机、建局选择、严格 `available_actions` 状态机、惰性计时、共享人类前端、多人 long-poll、平台公共动作与账号级防沉迷；固定夹具证明 `your_turn:true` 不再续刷 state、`your_turn:false + next_call` 可继续观察、同 revision 空观察有界等待、`rest` 不因 duel 指南缺词被 APK 阻断、暂停/暂离不触发远端弃权。中文目录标题能从实时 catalog 回填旧 session；多 session 面板区分 active/viewing。游戏房间不再调用 Gemini或显示跨 session 兜底提示。CI/真机仍分开登记；20 个 GitHub 审计不能冒充 29 个真实服务器存档逐局通关测试 |
...
### 2026-09-14 v0.41.74+218 Cedar 游戏厅全量协议审计与一次性适配（CI PASSED / APK READY / TRUE DEVICE PENDING）

1. 用户明确要求停止围绕双弈逐个症状补丁，完整读取每款游戏公开仓库并查清“通用接入缺口 / 游戏类型差异 / 个别例外”后一次施工；全工具调用动作展示延后，当前只收口 MCP 游戏厅。用户同时决定：Cedar 官网已经提供“允许小机自行重置”开关，APK 不得再另设能力硬门；房间 Gemini 若稳定失败则游戏厅专用对白改为 DeepSeek。
2. 审计覆盖 Cedar 当前 catalog 的 29 项：8 个测试类与海龟汤为 Cedar 站内实现，没有独立公开 GitHub；其余 20 个公开小游戏仓库逐一读取 README/协议。公开仓库为 duel/cedarduet、fishing、forest、bar、eco、ciyuwu、imitator_td、memoria、white_room、arcade、burger、market、leek、delve、travel、workkk、garden_cat、camping_plaza、crucible_echoes、moonlit。开发审计可读 GitHub，但角色普通盲玩仍只读实时 `get_guide`，仓库内容和谜题答案不进入伴侣 Prompt。
3. 类型结论：fishing/ciyuwu/leek 等简单单入口 `cmd` 游戏可由旧通用循环偶然工作；bar/arcade/burger/white_room/moonlit 等有必选建局参数；crucible_echoes/camping_plaza/imitator_td 必须严格消费服务端 `available_actions/legal_actions`；travel/eco/garden_cat/workkk 含惰性时间、共享网页或人类事件；duel/海龟汤/forest 是共玩、多方或长轮询。Cedar 只统一外层四个工具，不统一这些内部生命周期，旧 APK 没有协议适配层，才会出现“修一个入口、下一种状态又坏”，不是每个作者仓库本身都坏。
4. 防沉迷结论：官网 `/api/anti-addiction/machines` 文案明确是绑定小机级“连续玩 N 轮提醒、M 轮锁定、若干分钟后自动重置”，故 fishing 与 duel 共用累计轮次是服务端设计，不是把钓鱼存档字段错误复制到双弈。forest 的每日角色线、imitator_td 每 5 次决策提醒等属于游戏原生节奏，仍各自独立。实时 MCP `tools/list` 又明确 `play` 的跨游戏公共 action 包含 `rest / announcements / vote`；匿名协议探测确认 `play(game="duel", action="rest")` 返回“已重置，可以开新局了”。旧执行器却要求 action 必须逐字出现在 duel 自身指南，因 duel 指南不重复平台 action 而在本地拦截。+218 将公共 action 纳入平台权限层；服务端 `allow_self_reset` 开关继续作为是否准许 `rest` 的唯一裁决。
5. 双弈空转结论：备份中 revision 8 后人类落子使 revision 9 明确返回 `your_turn:true`，随后 APK 仍连续 20 余次调用 `state`。现有 `McpTurnStateResolver`只识别 `next_actor/current_actor/turn/seat`，漏掉 `your_turn`；增量包的 `current_actor` 只有 id/name、没有角色，故无法判定为伴侣。+218 增加布尔回合、服务端可用动作与只读观察语义；观察一旦确认轮到伴侣，下一 tick 必须进入动作规划，不能继续拿旧 `next_call` 当动作。相同 revision/相同只读结果须有界延后或暂停，不能无限 1 秒刷。
6. 暂离结论：当前 ActivityStore 已有本地 `pause/resume`，活动窗按钮可用，但 Agent registry 没有该工具；自然语言“退出”因只看游戏指南容易选择 duel `leave`，进行中即弃权。+218 增加可审计的本地活动管理能力：暂停当前、暂离并释放 active session、恢复指定存档；默认“我去忙/你先玩别的/暂停”不调用远端 `leave/resign`，只有明确要求离席、认输或永久退出才交给游戏动作。远端存档与旧 session 均保留。
7. 上下文结论：显式用户回合本来会看到当前普通聊天，但后台 `_advanceSession` 只含指南和本机局面，自主选游 Prompt 也只含 catalog 与“轻松新鲜感”；所以用户建议不会可靠进入后续决策。+218 不复制整段聊天，而把近期明确游戏建议/策略作为有界、可过期弱信号：自主选游可参考近期提到的候选但不当命令，当前局后台规划可参考最近建议但仍由她决定，服务端合法动作永远优先。
8. 中文 UI 结论：实时 catalog 已有中文标题；确定性 `get_guide` 与排队切换调用 `recordGuide` 时未传 title，session 因而回退英文 id。+218 建统一 catalog parser，从已缓存真实目录回填旧/新 session 中文显示名；活动窗不再横排无限 ChoiceChip，改为可展开会话面板，显示中文名、英文 id、phase，并分别标明“当前活动”与“当前查看”，活动边框使用主题主色。
9. 房间 Provider 结论：+217 备份设置 `cedar_room_last_final_provider_notice` 明确为“Gemini 房间回复失败（请求被拒绝 400）”；当前链每次先尝试 Gemini 至多两次，再 DeepSeek，随后把一条全局 setting 在所有游戏活动窗显示。诊断按隐私合同不保存中继响应正文，无法继续证明具体请求字段，但短房间对白无需承担全局最终 Provider 成本。+218 固定游戏房间对白为 DeepSeek，并只给它当前 session 的 pending message、动作、参数、Outcome 与有界建议；不再读取 8 条普通聊天、不再写/展示全局 fallback。普通聊天和沉浸房间的 Gemini 双通道保持原样。
10. 本批不为 29 个游戏各写硬编码业务客户端。生产实现按通用边界分层：动态目录/指南；平台公共动作；结构化 actor/continuation/legal action；只读与写入幂等边界；建局/用户输入；本地 session 生命周期；会话内房间消息。测试按生命周期 archetype 和已见真实 shape 建夹具，兼容矩阵逐项登记仓库能力；新增游戏只要遵守 guide/structured Outcome 即可进入，只有真正违反统一协议时才新增个别例外。
11. 实现结果：新增 `cedar_game_protocol.dart` 作为目录元数据、平台公共 action、只读动作与近期建议的统一合同；`McpTurnStateResolver` 直接识别 `your_turn / waiting_for_user / can_act / action_required / available_actions / legal_actions / legal_moves`。自主循环在伴侣回合重复同一只读调用时有界停下，平台 action 连续重复也有界延后；服务端返回的真实继续时钟和多人 long-poll 仍保留。
12. 防沉迷执行结果：Agent 用户回合与后台规划都允许 `rest / announcements / vote` 绕过“必须出现在单游戏指南”的旧本地误拦，但仍把请求和账号绑定交给 Cedar；即使当前没有本地 session，`rest` 也可真实送达服务端。已有 session 时平台结果作为独立事件记录，不覆盖棋盘/房间的 `last_action / last_outcome`，防止“重置成功却丢失局面”；失败后两分钟有界延后，成功后才唤醒原活动续接。
13. 生命周期与模式结果：新增 `cedar_toy.manage_activity` 的本地 `pause / pause_and_release / resume`，自然语言“我先忙、你玩别的”释放活动位并保留旧存档，不调用远端离席/认输；恢复会重新唤醒连续时钟。`co_play/multiplayer` 仍须明确许可；审计收尾发现旧枚举把 `hybrid` 一律当成必须邀请，现已拆成“支持共玩”与“强制邀请”，混合游戏可独自开始、进入共玩分支才需同意。
14. 目录/UI/上下文/Provider 结果：session 中文标题始终以实时 catalog 为准，并会覆盖修复旧的英文/模型标题；活动窗改为可展开列表，长名称省略显示但保留完整值，英文 id 与 phase 置于副标题，主色 2.2 边框标活动 session、容器色标当前查看，旧暂停存档可直接恢复。后台只保存最多三条、每条最多 500 字的明确游戏建议；房间对白不再读取全局最近八条聊天，固定走内部 DeepSeek，并清除旧全局 Gemini 失败提示。
15. 本地验证：新增 `cedar_game_hall_protocol_v04174_test.dart` 覆盖中文目录、平台 action、duel `your_turn:true/false`、严格合法动作、hybrid、建议持久化与暂离恢复；新增 29 项兼容矩阵和 +218 静态门禁。+218/+217/+216/+215 Cedar 专项、当前总账、workflow YAML、Python compileall 与 `git diff --check` 均通过。按 Actions 实际清单运行 98 个 Python validator，95 个本机通过；另 3 个分别因 Actions 才恢复的 417 桌宠/聊天视觉私有素材不存在和本机没有 `kotlinc` 而无法执行，没有业务断言失败。本机同样没有 Dart/Flutter SDK，故尚未声称 Analyze、Flutter tests、Kotlin/JVM、Release 或 APK 通过，必须由 CI 证明。
16. 功能、测试、兼容矩阵、工作流与本轮实现前后总账已提交为 `0e29ee3e24fe3cef6f5f5da73db25f598707837f`，tree `63ac50470aead41dcba92a8cd3a4f72db7f021c4`；公开提交不含用户上传备份/诊断、Token、房间 ID、私密棋局正文、模型资源、音频或 APK。尚未推送和触发 Actions，本项只记本地提交，不提前记 CI/APK。
17. 因本机 HTTPS 无 GitHub 凭据，改用已授权连接把相对 +217 远端 head `0f09c6d9c1f0710d96d840d4dee21f33268d838e` 的 32 个完整文件写入远端；1.59 MB 总账经 45 个 Base64 分片重组，blob SHA `8765e691ae3fe9ad48e684f8687e37011152dfbb` 与本地一致，最终远端 tree `fbdb980adea83e257cdecbcb6f1faf7c60c2a9a5` 与本地 HEAD tree 一致。远端提交 `c5afbc85fa9bd7d84e00fb0a5abc6887fdd1de47` 创建分支并触发 Actions run [`34833573954`](https://github.com/catkiss62/ai-companion-build/actions/runs/34833573954)（871）。首轮已通过全部私有载荷恢复、源码/历史 validator、Kotlin/JVM 与 Flutter Analyze；Flutter tests 为 `788 passed / 1 failed`，新 +218 九条夹具全部通过，唯一失败为旧 `cedar_toy_game_switching_v04169_test.dart` 的 `hasUserTurnContinuation`。核对发现失败样本是未批准 `coPlay` 而非此前口头误判的 hybrid：本轮拆分枚举时把 coPlay 的“仍需向用户取得许可”续接信号也误要求 `invitationApproved`。窄修保留 coPlay/multiplayer 未批准时的用户续接，hybrid 未进入共享分支时不误挂等，批准后才共享观察；同步新增三态断言。Release/APK 尚未开始，待第二轮 CI。
18. 窄修本地提交 `238fd575659b2448ef9205b710182adc942af643`，经 3 个 blob SHA 核对后写成远端提交 `3163aa6cfac80a14414baa8488950557f26a4245` / tree `7c6f2c7cd8ff5ff2f00678468df7cf6133f5a80e`。第二轮 Actions run [`34834400059`](https://github.com/catkiss62/ai-companion-build/actions/runs/34834400059)（872）完整成功：98 项源码/历史 validator、Kotlin/JVM、Flutter Analyze、`789/789` Flutter tests、arm64 Release、固定签名、四音色 Genie/Jiuhu/OpenJTalk 实包与每候选六输入、417 桌宠、LingChat、22 塔罗、checksum、Artifact 和未发布 Draft 全部通过。APK `AI-Companion-v0.41.74-218-Cedar-Game-Hall-Protocol-APK.apk` SHA-256 `3a6cb3a64d0e0374799165fe4e23d03e5042c133d5dac7efcf8823b4a3ec2d86`；Artifact `10343866886` 为 537,956,891-byte ZIP / digest `60670b868d2cf4f6affde63928677c64a24ee9c14babc3d3517357c4e6dbd47b`；Draft 为 `untagged-3f23ace4424267936d8f`。自动化不能替代 29 款游戏逐局真机通关，当前只可记 `CI PASSED / APK READY / TRUE DEVICE PENDING`。

### 2026-09-14 v0.41.73+217 双弈确定性入口与回复完整性热修（CI PASSED / APK READY / TRUE DEVICE PENDING）
...
19. 第三轮远端构建提交为 `0f09c6d9c1f0710d96d840d4dee21f33268d838e` / tree `ae394c33dda65bdb31d2dfde2966c4f0e30d7fe5`，与本地第三轮 tree 精确一致；Actions run [`34814623238`](https://github.com/catkiss62/ai-companion-build/actions/runs/34814623238)（870）完整成功：总账/范围、全部私有载荷与源码/历史 validators、Kotlin/JVM、Flutter Analyze、`780/780` Flutter tests、arm64 Release、固定签名、四音色 Genie/Jiuhu/OpenJTalk/ORT 实包、417 桌宠、62 LingChat、22 塔罗、checksum、Artifact 与未发布 Draft 全部通过。APK `AI-Companion-v0.41.73-217-Cedar-Entry-Reply-Completion-APK.apk` 为 544,825,782 bytes / SHA-256 `a82efc79e1e0ffe83a9478244d55b5f8a7967fd69d01d4c316a2cb1a72374dde`；Artifact `10335224449` 为 537,946,200-byte ZIP / digest `04b97b6a80e81fb40bd5dfe48f567d8912cea39d484f989b29374bbd711ae957`；未发布 Draft 为 [`untagged-564d938dac0a0133ebb3`](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-564d938dac0a0133ebb3)，含 APK、checksum 与 CI monitor。自动化通过不等于 REDMI K80 Ultra 真机已通过；下一步只做联合真机验收，不自动推进其他代码任务。

### 2026-09-13 v0.41.72+216 Cedar 信任优先、快速/观战节奏、即时分享与小酒狐七候选（CI PASSED / APK READY / TRUE DEVICE PENDING）

1. 用户纠正并锁定三档：活动窗每次打开默认“休闲模式”，单人已承诺游戏成功后目标间隔 2 分钟；“快速模式”5 秒、“观战模式”10 秒，两者都属于用户正在观战的临时模式，都应及时显示真实操作，notable/required 不再先进入 Thought 等待，而直接进入主聊天。关闭活动窗恢复休闲；加速选择不持久化、不改变她平时是否想开始新游戏的 Desire 竞争。共玩/多人依 Cedar 事件实时响应，不套单人节奏。
2. 最新 `17:00` 备份定量确认：钓鱼 session 为 `solo`，共 10 次真实动作跨约 10 小时 18 分，相邻记录中位约 5 分 33 秒、平均约 69 分；后半段虽有 2～5 分钟簇，仍非稳定两分钟循环。10 次中 2 次 notable，当前备份仅保留 1 条同主题 Thought，并有 1 次可见 `game_share` 投递。最后一次 Outcome 后被模型写成 `next_actor=user / waiting_user`，是单人自主游戏停止的直接证据。
3. 调用审计确认当前每个后台游戏步不是“纯 MCP”：`_advanceSessionLocked` 先以 DeepSeek 高思考读取完整 guide + local state 规划动作，MCP `play` 成功后 `_verifyOutcome` 又以 DeepSeek 重读完整 guide + Outcome 判断 actor/share；房间 message 再单独调用最终 Provider。钓鱼 guide 1161 字、单次 Outcome 117～763 字；双弈 guide 2663 字、完整局面 Outcome 2143 字。真正游戏分享则进入完整 `PromptBuilder`，包含人格/规则/记忆/世界书与最多 28 条近期消息，单次更重，但高频两次轻判断累积后同样不可忽略。+216 应删除“已有结构化 actor/terminal 仍模型核验”的重复调用，并避免稳态每步重复完整指南。
4. 永久方向以文件顶部“Cedar MCP 信任优先永久合同”为准：Cedar 是长期维护、面向大量人机恋用户且自带防沉迷与游戏规则的成熟服务，默认相信其 schema、指南、结构化 Outcome、合法动作、等待与终局；本项目只加与伴侣连续性、分享、隐私、授权、并发和可见事实相关的边界。不得再先堆普遍防御、出现阻断后逐项找原因；新增限制必须有真实失败证据并保持最小范围。
5. 实际实现：`CedarViewingPace` 定义休闲 2 分钟、快速 5 秒、观战 10 秒；活动窗打开显式重置休闲，快速/观战以可恢复设置+活窗心跳跨前后台保留，窗口关闭或新聊天界面构建时清回休闲。模式切换立即唤醒后台并可将旧的 2 分钟 due 提前；共玩仍依 Cedar 事件续接。
6. 不得回归：Cedar Token、房间 ID、私密手牌/答案不进入脱敏诊断；MCP 文本仍不能覆盖系统/人格规则；同一写操作保持单飞和真实参数对齐；用户主动 Stop、暂停、终局、明确服务等待继续有效；DeepSeek 内部 / Gemini 可见对话合同、TTS、schema 61、Snapshot protocol 6 不改。预定验证覆盖三档生命周期、solo actor、structured trust、短退避、写超时同步、即时分享排队、Gemini 请求方言和旧 +213/+214/+215 夹具。
7. 运行码收口：一旦 session 参与模式已立，规划器后续不再覆盖；Cedar 结构化 actor/terminal 直接生效，solo 无明确反证时归 companion，旧 `solo/waiting_user` 状态会就地修复。`resume_after_seconds` 由本地解析器直接取值；分享分类只读 Outcome、不再每步重读完整 guide。共玩写请求不再强制 `wait=true`，改为先收提交结果、再跟服务 `next_call`；长轮询空超时按正常续订，写入结果不确定时优先用已持久只读 continuation 同步，不盲重放。
8. 分享与 Provider：休闲 notable 仅留 Thought；快速/观战 notable 把同一 Thought ID 放入有界持久队列，立即调现有 `ProactiveEngine` 主聊天生成/真实性/落库/通知链；Active Brain、用户聊天租约、沉浸页和单 writer lease 仍会延后而不丢失。房间短台词改为 `thinking=false / low hidden thinking / maxTokens=512`，以避免旧 180 token + 思考请求的 Gemini 400；DeepSeek 兜底仍经机器 payload/Markdown/XML 清理。
9. 本地验证：+167～+172 Cedar 专项 validator、六个历史版本 allowlist、当前总账 validator、Python compileall、workflow YAML、`git diff --check`、Dart/Kotlin 括号与重复声明静态检查通过。本机无 Flutter/Dart SDK，且 Actions 才恢复的 LingChat/TTS/签名私有资源未在当前工作区，故未本地声称 Analyze/全量 Flutter tests/APK 通过；必须由 CI 完成。
10. 本地功能提交为 `8eb56c4f2e3774ccacd3d411f3dd4b7677609475`。尝试推送 `agent/v04172-cedar-trust-watch-modes` 时，当前环境的权限审查明确拒绝：它将先前用户授权解读为仅适用 +215，不允许自动延伸到本次 +216。未绕过审查、远端分支未创建、CI 未触发；需用户对“推送当前 +216 分支并触发 CI/APK”再给一句明确授权。
11. 构建前用户追加三段小酒狐 ASMR 视频候选，并明确要求：从视频提取 WAV、响度向现有四候选对齐；日语参考文本必须以实际听写为准，文件名中文翻译仅作校对，`【】` 内候选名不进入文本；追加顺序固定为“轻语、耳语、气声”，只允许手动选择，不联动“自动判断”。本任务继续并入尚未推送的 +216，不另增 build number。用户同时明确授权本窗口内 AI Companion 项目持续推送开发分支并触发 GitHub CI/APK，故第 10 项的权限阻塞已解除；授权仍不包含合并 `main`、发布正式 Release、删除分支/数据或公开用户视频、参考音频、模型权重及密钥。实现前须确认 Genie v0.7.6 六张参考特征的生成链与现有四段真实响度，禁止只加 UI 空选项或把原视频/WAV提交到公开仓库。
12. 音频处理真值：现有四条参考 WAV 均为 44.1 kHz / mono / PCM16 / `-26.4 LUFS`，故新增三条按同一响度基准处理。轻语源 4.830 秒，线性 `+2.4 dB` 后为 `-26.4 LUFS`、peak `-10.1 dB`，WAV SHA-256 `c5ba222a9613f94f85bf9e29dbabe5526d1c0ea8ead012fb9825800266d86c4`；耳语源为 stereo 6.664 秒，先下混 mono、线性 `+10.1 dB`，结果 `-26.4 LUFS`、peak `-9.8 dB`，SHA-256 `1de2171badd73e653b94c1c76bd95409dde82df6b61241bd884109fe64cb0918`；气声源 9.660 秒、原始仅 `-43.3 LUFS`，纯增益会削波，故采用 `+17.5 dB` 与最小限幅，结果约 `-26.3 LUFS`、true peak `-1.5 dB`，SHA-256 `67eb38b17a69cff9b71556dc59064b4eb1cc928d693b8d44897dea770d3ba9a4`。未做降噪、压缩或变调。
13. 日语参考文本由实际音频、多模型/分声道 ASR 与文件名中文提示交叉校对，不是把中文机械回译；`【】` 候选名未写入文本。最终为：轻语 `焦らないで、お楽しみはこれからなんだから`；耳语 `それとも、汗ばんだ制服姿の方が好き、とか？`；气声 `制服のままホテルに入るの、誰かに見られないか心配でドキドキしちゃったね`。耳语自动识别曾把低声段误并成 `汗ばんだ匂いのまま`，结合可听音节和文件名含义修正为 `汗ばんだ制服姿の方`；这是本次最需要人工校对的歧义点。
14. 私有生成链完成：从总账钉住的旧版精简包恢复 GPT SHA-256 `8a284b6754e2bc56b5931de1a3adceb77f3659ea2aef99a6e74dc88bb05e4fee` 与 SoVITS SHA-256 `4453038a042cfda3dde404174369cc25ff7ab56d401b3c1716d0579ec6e56621`，生成 prompt encoder 后以 Genie 的 `refresh_v2pro_references.py` 产出 `v0.7.7-jiuhu-v2pro-seven-candidates`。另一份同名精简包的 PTH 实际 SHA 为 `723ad144…` 且带原子写临时残片，不符合既有钉值，已作为失败路线弃用；大权重在 overlay 文件系统装载时还会异常截断，改在 `/dev/shm` 完整展开后生成，未把不完整权重带入结果。七条候选分别完成四 ONNX session 的 Encoder→自回归 Decoder→VITS 推理，均在 500 步门内停止且输出有限非空音频。
15. 应用实现把 `soft_speech / whisper / breathy` 按“轻语、耳语、气声”追加在既有四项之后，Dart 固定模式、Kotlin runtime case 与 `VoiceProfileCatalog` 三层同源；`_automatic` 仍只含日常/温柔/活泼/可爱，专项测试同时证明手动选择保持自身、19 个已知自动情绪均不会返回新增三项。运行资源 migration marker 升为 `ai-companion-v04172-build216-jiuhu-v077-seven-voices-v1`，覆盖安装会清理旧四候选版本并重新抽取，不会留下 UI 有选项、底层仍回落日常的空接线。
16. 私有构建不重复上传约 500 MB 主模型：新增未发布 Draft tag `genie-tts-private-runtime-v0.7.7-jiuhu-seven`，资产 `Jiuhu-v0.7.7-seven-candidate-prompt-overlay.zip` 为 8,506,935 bytes、SHA-256 `15126a2067b6da77af47b882fdd966e41ba84a38cc7eb6cbe274b38485cec0fd`，只含新增三条参考/六张特征、四条需同步刷新的 SSL 特征和七候选 manifest。工作流先恢复已钉 SHA 的 v0.7.6 四候选主模型 APK，再覆盖私有增量并校验 manifest 版本、候选顺序、三条日文、七模型哈希、每候选六输入与独立 prompt；随后照旧剥离原始参考 WAV 才打入 APK。Draft 保持未发布，公开 Git 只提交文件名、哈希和装配逻辑，不含视频、WAV、张量或模型。
17. 增量包不是只做 ZIP 列表检查：本地按 CI 顺序把旧私有 APK 的 `benchmark_jiuhu` 与增量 ZIP 合并，所得 70 个文件的集合、字节数和 SHA-256 与已验证七候选 bundle 全部一致；对合并结果再次运行七候选端到端推理，七条全部通过。工作流实际调用的源码 validator 本地为 93 项通过；另 3 项分别依赖 Actions 才恢复的 LingChat effects、417 文件桌宠源包和当前容器缺失的 `kotlinc`，均非本次源码失败。历史 +153 validator 仍断言队列重构前的 `prepared` 变量与旧工作流步骤全名，已只把断言推进到现行 `prepared.text`，并让新步骤名保留原历史前缀。+160、+172 Cedar、+172 三手动音色专项、当前总账、workflow YAML 与 `git diff --check` 均通过；当前容器无 Dart/Flutter SDK，故 Analyze、Flutter tests、Kotlin/JVM、Release、签名与最终 APK 实包仍只能由 Actions 证明，不能提前标 CI PASSED。
18. 三手动音色、私有装配、专项测试与实现阶段总账本地提交为 `6566f0ac`，父链中 Cedar 运行码提交仍为 `8eb56c4`。本机 HTTPS Git 因无凭据停止，未将其误判为 GitHub 故障；随后按用户持续授权通过已连接 GitHub 通道把完整本地构建 tree `a0a8dd27ca7c6a74de7b37279e3da5a36368996c` 写成远端提交 `33747687fe6b620530ba1dd6c11e710acc0a3caa` 并创建同名开发分支。远端 tree 与本地精确一致，`main` 未改。
19. Actions run [`34781049060`](https://github.com/catkiss62/ai-companion-build/actions/runs/34781049060)（867）完整成功：私有增量 `15126a…` 下载与合并、源码/历史门、Kotlin/JVM、Flutter Analyze、`779/779` Flutter tests、arm64 Release、固定签名、APK 内 66 项 Genie/Jiuhu/OpenJTalk 哈希、七候选每条六参考输入、ORT byte identity、417 桌宠、62 LingChat、22 塔罗、checksum、Artifact 与 Draft 上传全部通过。APK `AI-Companion-v0.41.72-216-Cedar-Trust-Watch-Modes-APK.apk` SHA-256 为 `9c0c76123d3701bea51edb9f1a9c41041adca3ef4914c8516913c5f318af688f`，签名证书 SHA-256 为 `305eb3d80983b963c64818ddf1ad561f279de6d47b3ed2c781ada448c7c25148`；Artifact `10324702656` 为 542,922,295-byte ZIP / digest `5f7544279fac390d17c9bbfe63da3ec03d6dd1fc8bf9c5b4dc3af07ff905b742`；未发布 Draft 为 `untagged-69730c44a344f0eee6fe`。自动化不能替代真机：Cedar 三档、共玩/Gemini 和新增三音色实际听感仍为 `TRUE DEVICE PENDING`。

### 2026-09-13 v0.41.70+214 Cedar 实时共玩与房间对话真机失败取证（TRUE DEVICE PARTIAL · +215 HOTFIX STARTED）

1. 用户覆盖 +214 后证明基础落子链已较 +213 前进：她真实加入用户建立的双弈房间，首步 `move` 实际成功，发起方不再反转。但首步返回“轮到用户”后，App 结束了当前聊天生成，没有继续持有房间观察；用户在 Cedar 网页落子与发言均已到达服务器，但直到用户在 App 主聊天再说“我下好了”才重新启动 `state`。因此 +214 仍是“用户消息驱动的单轮 Agent”，不是双弈需要的持续房间 Agent。
2. 真实 duel 指南已明确：非己方回合使用 `state(wait=true)`，己方 `move(wait=true)` 落子后继续等待；这是“当前 MCP 请求内挂等”，不是服务端推送，上一条请求结束后 Cedar 无法主动唤醒 App。服务结构化 Outcome 会返回 `next_call`，+215 必须把它持久化为可恢复的观察指令，由现有前台服务/恢复协调器持续执行，而不是让用户反复去主聊天提醒。
3. 本次 `state` 真实返回中，完整局面位于 `room`，`room.current_actor` 明确是她，但 +214 `McpTurnStateResolver` 只查根、`snapshot/state/data/result`和单个 `rooms[]`，漏掉单数 `room`，所以再次落为 `next_actor=wait / waiting_remote`。这是通用嵌套结构遍历缺口，不是五子棋策略或模型智力不够。
4. 同一真机轮最终消息只有 `"revision": 3, "wait": true, "move": {"row": 5,` 这段残缺参数，工具 Outcome 表最后仍只有 `cedar_toy.play/state`，没有第二次 `move`。代码路径显示：工具结果后的迭代规划一旦返回零 structured tool call，`while` 直接退出并把该 DeepSeek 规划 content 当最终正文，没有再进 `generateFinal`；而现有机器协议守门只识别 DSML/tool_calls 标记，不识别这种无头的参数 JSON 片段。故不能只继续扩正则；所有规划路径都必须显式转入最终表达阶段，并在落库/TTS 前对孤立 JSON/XML/参数片段作最后防火墙。
5. 首次真实落子为 `(6,6)`，同轮可见正文却声称 `(7,8)`；真实 Outcome 和远端棋盘支持前者，不支持后者。+215 需在最终表达中注入已执行的准确工具参数/Outcome，且对落子、房间、身份等可核验声明以本轮工具真值为准，不允许最终模型改写已发生的坐标。
6. 网页 Outcome 已包含用户在房间发的消息事件，指南也明确 `join/move/state/resign/leave` 支持与 `move` 同级的 `message`。因此房间对话应通过 MCP 结构化事件和参数双向桥接，让她直接在 Cedar 页面一边玩一边说；禁止用无障碍/DOM 注入伪装点按网页。房间消息是最终可见对话，仍遵循 DeepSeek 规划与 Gemini 最终表达分层；Gemini 失败时只允许经同一可见防火墙的 DeepSeek 自然语言兜底。Cedar 官方入口为 `https://toy.cedarstar.org/`，已在既有客户端基址合同中记录，本节再作网页交叉验证基准。
...
12. 兼容窄修远端提交 `b4df41bfb9f7dee7fd30ce3725e51b30153d1be6` / tree `0d640384b8c5d76a9ead10be7f59eabf019d263f`。第二轮 Actions run [`34765341286`](https://github.com/catkiss62/ai-companion-build/actions/runs/34765341286)（865）已通过全部 Source/regression validators、Kotlin/JVM 和 Flutter Analyze；`773/774` Flutter tests 通过，唯一失败是 +214 旧夹具要求“混合内容中清除机器协议但保留独立自然正文”，+215 新增的整体快速拒绝误把后半句正文也清空。这不是 JSON 残片泄漏，而是守门过度删除；窄修只移除 `removeUnsupportedSentences` 的“整段提前返空”，保留每句 `looksLikeMachinePayload` 拦截。因此纯残片仍为空，混合内容仍可保留真实自然语句；待第三轮全量 tests 证明。
13. 最终运行码远端提交 `b9e5980a43627f77e4f211d9f49a02ccbd67de51` / tree `6f70775af50c8bf75bd6257a339483495cec159e`。第三轮 Actions run [`34767401057`](https://github.com/catkiss62/ai-companion-build/actions/runs/34767401057)（866）完整成功：全部 Source/regression validators、Kotlin/JVM、Flutter Analyze、`774/774` Flutter tests、arm64 Release、固定签名与全部私有资源校验通过；APK、checksum、Artifact `10321340130` 和未发布 Draft Release 均已生成。APK 为 544,811,306 bytes，SHA-256 `f7b9ba7606ef34dde10de243f3d7da496894ea4ee6bfd91a519e5421ce49d018`。当前状态只可记 `CI PASSED / APK READY / TRUE DEVICE PENDING`，等待覆盖安装实测实时接招与房间对话。
...
1. 用户提交本轮脱敏备份与诊断。证据来自 +213 真机，不是 +212 旧存档：`cedar_toy_activity_state_v2.active_game_id=duel`，duel 与 fishing 会话已分开保存，目标双弈完整指南、`rooms` 和 `state` 均真实成功。因此 +213 的多游戏状态、目标指南切换与远端只读同步是正样本，但完整共玩闭环未通过；附件、账号、房间与玩家标识均不进入公开仓库。
2. 用户已经明确表示自己开好房并向她发出共玩邀请；总账 +213 合同也明确这种情况应直接视为 `invitation_approved=true`。实际首轮模型却给出 false，本地 `_cedarPlay` 对所有 requires-invitation action 统一拦截，写入 `awaiting_invitation` 和“等待你接受共玩邀请”，并产生 `cedar_invitation_required` blocked Outcome。于是人物回复变成她反向邀请用户，发起方语义被颠倒。当前实现只信模型布尔值/旧会话，没有把“用户发起邀请、已给房间码或已开房”作为确定性的双边同意事实，违反已写合同。
3. 用户再次确认开始后，真实 `rooms` Outcome 返回现有房间由“对方新建”，状态 playing，`turn=ai` 且 own seat 与 current actor 一致。这只证明轮到 AI，并不证明 AI 执黑或先手；可见回复却说“我执黑先手”。用户纠正后，真实 full `state` 更明确给出：用户为 human、token X，已经落下黑子；她为 bound_machine/AI、token O，且 `current_actor` 正是 AI。所以用户对棋局的描述完全正确，错误来自 App 对真实 Outcome 的表达与状态归类，不是用户操作或 Cedar 网页。
4. `_verifyCedarOutcome` 目前把完整指南与 Outcome 再交给 DeepSeek 自由分类，失败或误判都保守回退 `next_actor=wait`。本次它把上述明确的 `current_actor=AI` 保存成 `next_actor=wait`、phase=`waiting_remote`、waiting reason=`等待游戏允许继续`，从而关闭后台续步。对含 `current_actor / participants / role / kind / token / status / revision` 的结构化 Outcome，应先由本机确定性提取行动方、终局与身份；只有通用字段无法判断时才使用 DeepSeek 核验，不能让模型覆盖明确协议事实。
5. 用户纠正后，本轮第一步 `state` 是真实 structured tool call，Outcome 成功；模型的 reasoning 也正确读出她执 O、用户执 X 且现在轮到她，并形成合法的相邻落子意图。但第二次规划响应把调用编码为 `<｜｜DSML｜｜ calls>...` 文本，`generated.toolCalls` 为空。`DurableGenerationRunner` 只在进入 while 前处理一次“toolsOpen 但零 structured tool calls”，没有处理工具结果后的后续规划轮；while 因零 toolCalls 直接退出，DSML 文本被当成最终 content。`ChatSegmentCodec` 随后逐行存成 dialogue，真实性门也没有机器协议正文检测，因此用户看到整段代码。
6. 工具 Outcome 表最后一项仍是 `cedar_toy.play/state`，没有 `move`；远端 revision 没有推进、棋盘也只有用户的棋子。故 Cedar 网页没有继续下棋是正确反映：计划中的落子从未离开 App。下一热修必须在每个规划轮都统一处理“structured tool_calls / 可识别 DSML 兼容封装 / 普通自然文本”三态：机器调用只能经本地 registry、参数、指南、预算与重复调用 Gate 后执行；解析失败时明确终止或重试，任何 DSML/JSON/XML/tool-call 标记都不得提交为消息正文。
7. 同一备份还显示，DSML 泄漏消息后约 1.18 秒又送达一条旧 fishing 主动分享。该分享来自此前钓鱼 notable Thought，并非当前 duel Outcome；它在用户正进行共玩回合时插入，进一步打断游戏。下一热修应让“当前用户回合正在执行或存在活跃 co-play 且轮到任一参与者”成为游戏分享的投递门，旧单人游戏分享延后而不是丢弃，也不提高普通主动联系频率。
8. 下一修复包限定为：双边邀请事实识别；结构化 Cedar actor/participant/terminal 状态优先解析；每轮工具响应的 DSML/机器协议防泄漏与本地执行闭环；基于真实 Outcome 的棋子/先后手表达约束；活跃共玩时延后旧游戏分享。不得针对五子棋或 duel 硬编码落子策略，不修改 Cedar 服务、TTS、schema 61 或 Snapshot protocol 6，不把模型生成的 move 绕过现有本地 Tool Gate。当前只完成取证与任务登记，运行代码、测试、版本、CI 和 APK 均未修改；后续不得把本节写成已实现。

### 2026-09-13 v0.41.69+213 Cedar 跨游戏切换与后台续步热修（CI PASSED / APK READY / TRUE DEVICE PARTIAL · CO-PLAY LOOP BUG FOUND）
...
3. 活动状态升级为 Setting JSON v2：最多保留 8 个按 game 隔离的 `CedarGameSession`，另存 `activeGameId`、可靠 `QueuedSwitch`、跨游戏提醒与短期 `GameExecution`。继续镜像写旧 v1 当前会话，便于回滚；不改 SQLite schema 61，不改 Snapshot protocol 6。旧钓鱼进度不会因读取双弈指南被覆盖或删除。
4. 用户明确要求切换时，当前网络中的单次原子动作允许完成，不暴力取消；目标 game 写入可靠队列，不依赖概率 Thought。目标完整指南取得后才切为 active 并按该指南判断模式/动作；用户“我已开房，快进来”和房间码属于明确共玩邀请，可作为 `invitation_approved=true`，无需机械地反向再邀请一次。失败、尚未取指南或尚未真实 play 时只能说正在处理/已排队/尚未进入。
5. 普通用户轮不再因为任意 solo 游戏未结束就永久开放 Cedar 工具，避免无关聊天暗中推进钓鱼。显式“游戏厅/Cedar Toy/一起玩”仍触发；共玩、等待用户的活动仍能自然续话。若目标与当前 game 不同，同轮工具循环同时允许 `get_guide`，当前指南在 Prompt 中明确只授权自身 game。
6. “是否继续玩”分为两层：新开游戏仍是现有 Desire/Thought/Intent/Gate 的 `play_game` 候选，20 分钟冷却只约束再次自主选新活动；一旦活动已承诺且真实 `next_actor=companion`，不再等待下一条用户消息或重新参与欲望竞争。后台恢复协调器读取独立 due clock，默认约两分钟推进一个原子步骤：先由 DeepSeek 依据指南选 action/params，MCP 返回真实 Outcome 后再由一次 DeepSeek 内部核验决定 `next_actor/share_level`，绝不在 Outcome 前猜轮次；无效模式/动作延后五分钟，核验失败则保守暂停且不重放已经发生的远端动作。
...
21. 用户点名授权 +212/新分支后，通过已连接 GitHub 以 +211 远程 head `3f02731a` 为基线顺序写入三个提交：`ef23abb8`（持久游戏 Agent loop）、`49a22591`（活动窗/沉浸静默/情绪窄修）、`31ce3ab1`（版本/测试/workflow/总账）。中文总账因 1.5 MiB 普通传输曾被截断，在更新分支前已截住并改用 8 块传输；最终每个 blob 与 tree `08996fa558bb93a336a892146113558af5aee7b4` 均与本地目标哈希完全一致。
22. Actions run [`34718260051`](https://github.com/catkiss62/ai-companion-build/actions/runs/34718260051)（857）完整成功：专项/历史源码门、Kotlin/JVM、Flutter Analyze、`756/756` Flutter tests、arm64 Release、固定签名、Genie/Jiuhu/OpenJTalk/ORT byte identity、417 项桌宠源、62 项 LingChat、22 张塔罗、checksum、Artifact 与 Draft 均通过。APK SHA-256 为 `ae37c7c9638476a6ef9ab98c953ff549e463b51b1d3e0aa44d472dc0ef82293e`；Artifact [`10305163245`](https://github.com/catkiss62/ai-companion-build/actions/runs/34718260051/artifacts/10305163245) 为 537,881,600-byte ZIP / digest `050ad4211f6e5ac4e689ceadc569b98b0d7d34cb51ef941575dc385058f8dee8`；[Draft Release](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-f513d08d88a1c68308fa) 未发布。自动化通过不等于 REDMI K80 Ultra 真机验收通过。

23. 真机随后验证了单人钓鱼可以调用，但暴露两项阻断问题：活动不会在用户静默时连续推进；钓鱼进行中用户邀请进入双弈并提供房间码后，系统没有先取得 duel 指南，而连续以 fishing 指南校验目标动作并被阻止，回复仍误称即将进入。该失败已成为 +213 热修的直接证据，+212 不得再标记为完整真机通过。
...
14. workflow 触发窄修的本地提交为 `9ae0240`，授权 GitHub 连接写入远端为 `7e2faf294858633921fc1b2d5f941052f6b008a7` / tree `86318d0471858bfd935610ef7fd7f326c37ada65`。run [`34658611917`](https://github.com/catkiss62/ai-companion-build/actions/runs/34658611917)（850）完整成功：源码/历史 validator、私有载荷固定 SHA、417 项桌宠、62 项 LingChat、22 张塔罗、Kotlin/JVM、Flutter Analyze、`745/745` Flutter tests、arm64 Release、固定签名、Genie/Jiuhu/OpenJTalk/ORT byte identity、checksum、Artifact 与 Draft 上传全部通过。APK `AI-Companion-v0.41.66-210-Phase3C-Phone-Pacing-APK.apk` 为 544,724,654 bytes / SHA-256 `ba13db4d875a0fab7a7ca0902aab9097f6fcf05309a9a604cca5b59a677cc0b5`；Artifact `10285589701` 为 537,845,978-byte ZIP / digest `dde22b98f26da9210756b40a52c435275c9a61211e71fa81436e875f4a516c5b`；未发布 Draft 为 `untagged-43dc2e10769542425043`。`main` 未改，自动化通过不等于真机自然节律或 Phase 3 独立审查已通过，Phase 4 仍被这两项证据门阻塞。
...
19. 授权连接把本地最终 tree `dd0066b832b4ca6af1c5a91805e2d187b479b847` 精确写入公开开发分支，远端构建提交为 `cc5286df0810db565d9d15879700c73efcbd9836`；未触碰 `main`。Actions run [`34651532509`](https://github.com/catkiss62/ai-companion-build/actions/runs/34651532509)（849）完整成功：全部源码/历史 validator、Kotlin/JVM、Flutter Analyze、`735/735` Flutter tests、arm64 Release、固定签名、Genie/Jiuhu/OpenJTalk 与 ORT byte identity、417 项桌宠源、62 项 LingChat、22 张塔罗、checksum、Artifact 与 Draft 上传全绿。APK 为 544,709,718 bytes，SHA-256 `8e37131040a36d53d3982ca70aa9fa31e148cd6df8b277d0f2eac03d3c5afcd3`；Artifact [`10283904266`](https://github.com/catkiss62/ai-companion-build/actions/runs/34651532509/artifacts/10283904266) 为 537,831,077-byte ZIP / digest `sha256:6fe69d109bdccecec8c8acb38a5ca9058c6e9dc4d78c9954735b21382cca0169`；[Draft Release](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-1160d6f273e360a517f2) 保持草稿。自动化只记 `APK READY`，三入口视觉、刷新/截断交互、愿望/随笔、媒体复用与沉浸括号仍为真机待验；当前下一主任务提升为 Phase 3C，Phase 4 等 Phase 3 独立审查后再进入，T1 继续保留。

### 2026-09-11 v0.41.64+208 设置导航整体回退到 +206（CI PASSED / APK READY / TRUE DEVICE PENDING）

1. 用户明确指出 +207 对设置结构的理解错误：侧边栏里的主动联系、聊天画面、语音与情绪、文字演出都是故意保留的高频入口，不能因为与总设置页共享同一份配置就删除；用户也不会去底部“更多”寻找这些常用设置。底部“更多”不应复制完整设置列表，侧栏“全部设置”的旧标题、旧注释和 `/settings` 跳转也不得擅自改写。
2. 用户要求为了避免继续误判，先把**所有设置相关内容**完整还原到上一版，再由用户逐项说明后续改法。本次因此不保留 +207 的任何设置 UI/导航/说明/交互改动：`app.dart`、普通聊天侧栏、聊天快捷设置页、底部“更多”、设置分类页、总设置页以及两项对应 Widget 测试均按 +206 commit `36e674597a9c579aec86d555217ab4dd658ec8b0` 原样恢复。+207 的 TTS 流式队列、语义音色、原生诊断等非设置核心不回退。
3. 精确校验已通过：上述八个设置相关源码/测试文件对 +206 执行 `git diff --exit-code` 返回 0；`validate_v0360_ui_relationship_age.py` 同步恢复旧五分类合同；+207 专项验证器改为锁定恢复后的五分类“更多”、六域总设置、四个侧栏快捷入口与旧“全部设置”注释，同时继续验证 TTS/诊断核心。专项静态门通过。当前容器无 Flutter SDK，无法本地运行 Widget tests/Analyze；全仓历史 validator 直接全量遍历会因 CI 才恢复的私有资源、缺少 Kotlin/Flutter 以及大量退休版固定版本门产生预期失败，不能冒充本次回归失败。
4. 用户随后要求先构建 APK 看到还原结果；本批因此升为 `v0.41.64+208`，新分支为 `agent/v04164-settings-rollback`。新增 `validate_v04164_settings_rollback.py`，用八个 SHA-256 锁死 +206 设置源码/Widget 测试字节，并额外验证“更多→数据与高级→七大规则”的可达路径。仍不混入“正常完整回复也显示刷新按钮”、日语 TTS 根修、CPU 快速档或新的设置设计；这些必须等用户看到基线后再逐项明确。
5. 首轮 Actions run [`34633190974`](https://github.com/catkiss62/ai-companion-build/actions/runs/34633190974)（846）通过总账目标哈希、版本/schema、两私有 TTS 载荷、417 桌宠、LingChat/塔罗资源与签名恢复；源码回归门中 v0.41.28～42 全部通过，随后 `validate_v04143_phase3b_question_autonomy.py` 因允许版本止于 `63+207` 失败，未进入 Kotlin/Flutter/Release。该失败与设置源码、七大规则、编译和资源均无关；已把 v04143/45/47/48/49/50 六个同类历史允许列表统一推进到 `64+208`，本地逐项全绿，下一轮必须从完整流水线重新验证。
6. 修正提交 `3a389698faaa55a2808b3e3ab3d2e125461e06b4` 触发第二轮 Actions run [`34633781191`](https://github.com/catkiss62/ai-companion-build/actions/runs/34633781191)（847）并完整成功：全部源码/历史 validators、Kotlin/JVM 与 Flutter debug、Flutter Analyze、`731/731` Flutter tests、arm64 Release、固定签名、48 项小酒狐/Genie/OpenJTalk 实包身份、ORT byte identity、四独立候选、退休语音排除、OpenJTalk ELF、塔罗、checksum、Artifact 与 Draft 上传全部通过。APK `AI-Companion-v0.41.64-208-Settings-Rollback-APK.apk` 的 SHA-256 为 `2eb9fda6d6008918694c1993fc9ac649cc9ae432bb9b894908085782b19f16b7`；Artifact `10277803883` 为 537,820,981-byte ZIP / digest `f120d784ff9372729d3e66e2d01a70da89a1d471b1814b69be8cf3024a635ae5`。Draft Release 为 `untagged-9e86d80803d2918eae32`；成功后工作流把总账差量物化并删除临时 patch，远端分支 head `7957b750d7514114adaa2abadd3c5a2fd6c5eb44` / tree `943e6a8a84b4c9046a317adf2fd2ad175bb0f789`。自动化只证明代码/资源与可达性，用户实际看到的设置布局仍标 `TRUE DEVICE PENDING`。

### 2026-09-11 v0.41.63+207 沉浸低延迟朗读、音色语义、设置归一与路由硬化（CI PASSED / APK READY / TRUE DEVICE PENDING）

1. 用户接受当前运行记录的有界清理事实：`desire_events` 写入时删除 14 天以前记录，`device_events` 由维护任务按 21 天且最多 12,000 行裁剪；短时间高频导致备份暂时增大可以接受，只要求冗余不会永久无限增长。故 +207 不改变 schema、不清空历史、不新增删除按钮，也不把参与近期设备情境判断的事件与纯诊断事件误用同一删除策略；后续只有出现无界增长或备份负担的真机证据才重开。
2. 本批确定交付沉浸中文流式 TTS：仅当开启自动朗读且当前语言为中文时，在 LLM SSE 中按结构累计，首个完整句或闭合对白一旦形成即送入现有有界生成/播放队列，不等整条正文完成。日语/英语继续等完整中文正文后做一次 DeepSeek 投影，避免按分片重复请求和翻译漂移。正常完成后不得再次整段 `playText`；停止、失败或异常断流立即取消本轮队列，未闭合尾部不生成，截断草稿仍由 +206 的“重新生成/确认回复”决定是否进入上下文与摘要。
3. 全文朗读从单一扁平字符串升级为带语义的朗读单元：`ChatSegmentCodec`/沉浸段落判定产生 dialogue 与 narration/action；全文模式下旁白和动作永远用“温柔”候选，对话 `「」` 使用用户所选音色或自动情绪音色；仅对白模式继续只使用已选/自动音色。不得按字体颜色反推语义，避免主题或渲染变化破坏 TTS。
4. “温柔”候选增加固定相对倍率 1.2：最终有效速度=`clamp(用户速度×1.2, 0.5, 2.0)`，其他候选仍为用户速度。设置页继续显示用户的全局倍率，因此 1.0 时温柔实际 1.2，1.2 时温柔实际 1.44；不把存量 setting 自动改写为 1.2。原生播放必须在不同有效倍率边界等待已缓冲 PCM 排空后再切换 `PlaybackParams`，不能让后一个动作段的速度回溯污染已经写入的对白；同倍率相邻片段继续无缝写入。
5. +206 源码审计已经确认双 Key 主结构正确：legacy `readApiKey/readEndpoint` 固定 DeepSeek，final lane 才会按选择读取玩游；`ProactiveEngine` 直接读取 DeepSeek，因此当前主动消息不会调用 Gemini。发现的真实 UX 风险是提供商下拉只修改页面临时状态，用户切回 DeepSeek 后如果未再点“保存”，持久 provider 仍可能是 Gemini。+207 把“选择 DeepSeek”做成立即落盘 kill-switch；选择 Gemini 仍需填写独立 Key 并明确保存，防止误触发固定积分请求。补静态/单元门覆盖普通、沉浸、主动、Agent、翻译、摘要与 post-turn，不允许内部调用读取 final Key。
6. TTS 日语问题本批只增强取证，不猜修：诊断按不泄露原则增加请求语言、音色、消息/段落索引、规范化文本长度与 SHA、音素数量/范围/哈希、语义向量数量/哈希、frontend/acoustic/infer/wav 粗阶段和异常类型；不记录正文、PCM、参考音频或 Key。现有真机矩阵“fresh→zh/en 后首次切 ja 失败，fresh→ja 可用”与中文段落偶发日语听感必须分开归因；+208 以完整 runtime 重建矩阵单独修复，避免与低延迟队列同时改变后无法判断根因。
7. CPU“超频”同样后排单独 A/B。当前 ORT 固定 8 线程且真机 PSS 曾超过 2 GB，盲目增加线程可能提高发热、降频或触发 isolated service/binder 失败。后续若做，必须提供稳定/快速显式开关，分别记录冷启动、暖机、持续多段 RTF、PSS、温升和失败率；本批不偷偷提高线程或持有三语前端。
8. 设置导航以底部“更多”为唯一权威中心：先复用现有页面类，把“她/你们”和七域模型、记忆、主动、语音、表情包、设备与数据、诊断全部放入同一入口并逐项核对旧能力；侧栏保留查手机、沉浸房间、她的状态、世界书及素材说明等高频/独有快捷方式，删除主动联系、聊天画面、语音与情绪、文字演出四个重复分类，侧栏“全部设置”只切换到底部“更多”，不再建立第二套设置状态。
9. 版本目标 `0.41.63+207 / schema 60 / Snapshot protocol 6`。不得回归 +206 的 Gemini 两次有界尝试、DeepSeek 兜底持久提示、异常断流安全草稿、刷新确认、三语投影缓存、紧凑语言按钮、小酒狐四候选、单播放与停止链；动作/神态括号仍按模型偶发格式观察，不叠加 Prompt 禁令或字符串补丁。实现后必须通过新增专项、历史 validators、Kotlin/JVM、Flutter Analyze/全量 tests、arm64 Release、签名与 48 项私有 TTS 实包验证；自动化成功只标 `CI PASSED / APK READY`，不得替代真机低延迟、听感、发热和语言切换证据。
10. 本地实现已把可见文本预处理升级为 `TtsPreparedUnit(text, role)`，全文模式保留 `dialogue/narration` 语义而不再提前压成单一字符串；队列按单元选择音色，narration 固定 `gentle`、dialogue 使用会话开始时解析的所选/自动音色。所有 gentle 单元向原生播放层传相对 `1.2`，`NativeTtsEngine` 再与用户全局速度相乘并夹到 0.5～2.0；`WavAudioPlayer` 的每段命令携带最终速度，只有速度变化时才等待前序已写 frame 播放完后切 `PlaybackParams`，同速片段继续沿用单一 `AudioTrack`。
11. 新增 `TtsStructuredStreamParser`，按增量识别沉浸行首 `「/“/"` 为对白、括号或无引号独立叙述为旁白，流中只在句末标点、闭合引号/括号或换行时产出；只有收到完整终止后才由 `finish()` 释放无标点尾部。沉浸 controller 在中文+TTS+自动朗读时于首个单元解析后启动队列并用当时已生成正文做本地情绪判断；成功后 `endStream` 且跳过旧整段重播，Gemini 截断、连接异常、用户 Stop 或重新生成失败路径均先 `stop` 并丢弃未闭合尾部。日/英仍保持完整消息后一次投影/朗读。
12. TTS 取证已贯通 Dart→MethodChannel→主进程→隔离进程：每个声学分段携带 index；主进程只记录规范化输入字符数与 SHA-256、请求语言/音色、WAV 字节数；隔离 runtime 在 frontend 后记录音素数量、min/max 与序列 SHA，在 infer 后记录 semantic token 数和既有语义 hash，并把粗阶段写入跨进程 checkpoint。成功 `generation_ready` 与失败 `generation_failed/child_process_exit` 都能合并这些字段；`RuntimeDiagnosticStore` 仍是最多 160 条、30 天的脱敏环，不写正文、PCM、参考音频、路径或 Key，因此不会无限增长。
13. 双 Key 审计未发现 +206 当前运行时的主动消息串线：`ProactiveEngine` 与 internal/Agent/NSFW/摘要/翻译继续读取 `readApiKey/readEndpoint`，它们已硬绑定 DeepSeek；只有普通与沉浸 final expression 读取 `readFinalReply*`。实际补丁针对设置保存窗口：下拉一旦选 DeepSeek就立即启动安全存储写入并显示“后续回复不会再调用 Gemini”，而选 Gemini 仍只在完整保存与独立 Key 校验后启用；显式保存会先等待任何在途 kill-switch 写入，避免快速切换的异步写顺序反转。
14. 设置导航实现复用一个公开 `SettingsDomainList`：底部“更多”保留“她/你们”，并直接展示模型与联网、记忆与成长、主动联系与感知、语音与聊天呈现、表情包、设备与数据、诊断与开发七域；现有具体页面和 setting key 完全复用。Material 3 侧栏继续保留查手机、沉浸房间、她现在的状态、世界书和素材说明，删除四个重复分类设置卡；“全部设置”关闭侧栏后由 AppShell 切到底部 More，不再 push 第二导航中心。旧 `/settings` route 与页面类暂留兼容已有深链，但不再是主入口。
15. 本地新增 +207 专项锁定 provider kill-switch/主动 DeepSeek、闭合句流式生命周期、角色音色与 1.2 倍率、PCM 倍率边界、脱敏诊断字段和唯一设置中心；新增 parser、processor 与 queue 单测覆盖中断尾部、旁白/对白和 gentle 倍率。当前容器无 Flutter/Dart SDK，Gradle wrapper也因外网限制无法下载 Gradle，故不能冒充 Flutter/Kotlin 编译已通过。已通过 +207/+206/current-ledger 专项、workflow YAML、`git diff --check`，以及 Actions 同款 86 个 Python 源码门中所有可本地执行项；仅 LingChat 私有恢复、417 桌宠恢复和本机 `kotlinc` 三项因资源/工具缺失不能运行，必须由 Actions 真环境完成。
16. 首次远端同步误以总账中的 +206 APK 构建 head `3bbc438b` 作为分支最终基线，遗漏随后 `[skip ci]` 的总账回写 head `5f43ebed`：run [`34613845398`](https://github.com/catkiss62/ai-companion-build/actions/runs/34613845398)（839）在产品编译前被总账差量基线哈希拦住；以真实最新 tree 快进修正后，run [`34614038301`](https://github.com/catkiss62/ai-companion-build/actions/runs/34614038301)（840）按文档同步正确跳过 APK。正式 run [`34614128941`](https://github.com/catkiss62/ai-companion-build/actions/runs/34614128941)（841）已通过总账重建、版本/schema、私有小酒狐、417 桌宠、LingChat 与塔罗恢复，但历史 +155 验证器仍断言旧 UI 文案“不使用真流式测试模式”，与 +207 已明确启用沉浸中文闭合句流式朗读冲突，故在编译前失败；已把该历史合同精确推进到新文案并保持普通聊天 controller 不启用流式的原约束，下一轮须重新覆盖完整 Kotlin/Flutter/Release 门。
17. 修正后的 run [`34614718484`](https://github.com/catkiss62/ai-companion-build/actions/runs/34614718484)（842）通过完整源码/历史 validators、Flutter 依赖解析、Kotlin/JVM 与 Flutter debug 编译及 Flutter Analyze；全量 Flutter tests 为 `728 passed / 3 failed`。三项失败均是设置归一后未同步的旧测试期望：仍要求底部“更多”旧五域、`/settings` 旧六域与 +206 build label，而产品 UI 已有她/你们加七设置域且唯一中心文案、Agent Self 已为 +207。已把三项测试推进到真实 +207 信息架构和 build label，不改产品实现；下一轮必须先证明三项失败消失，再继续 Release/签名/实包门。
18. 最终 run [`34615837829`](https://github.com/catkiss62/ai-companion-build/actions/runs/34615837829)（843）完整成功：正确总账目标、固定私有资源、全部源码/历史 validator、Kotlin/JVM 与 Flutter debug、Flutter Analyze、`731/731` Flutter tests、arm64 Release、固定签名、48 项小酒狐/Genie/OpenJTalk 实包身份、ORT byte identity、四独立候选、退休语音排除、OpenJTalk ELF 闭包、塔罗、checksum、Artifact 与 Draft 上传均通过。APK `AI-Companion-v0.41.63-207-Immersive-TTS-Settings-APK.apk` 为 544,697,546 bytes、SHA-256 `6f7c36f93a915512b54ed72f057699d513953acca7b50198a2ce1be9b94b743b`，与 Draft 服务端 digest 一致；Artifact `10270877076` 为 537,820,065-byte ZIP / digest `337a7ad02a63530c306b7f0d0f98f8615cad0f2c6bbd8f6f5dd6608ede1dc1c9`。成功后 CI 应用并回写 hash-locked 总账、删除临时差量，分支 head `df6f5db49da66352793846637dde7e75a93e77e4` / tree `427684b669379dfceaf56d11138dec487eb74709`；当前严格状态为 `CI PASSED / APK READY / TRUE DEVICE PENDING`。

### 2026-09-11 v0.41.62+206 双通道最终表达、兜底与安全截断草稿（CI PASSED / APK READY / TRUE DEVICE PARTIAL）
...
3. +204 源码已确认可恢复目标为 `POST https://wy.aiwangyou.cc/v1/chat/completions`、模型 `[特价]gemini-3.7-flash-0.5`、独立安全槽 `aiwangyou_gemini_api_key`。只恢复 endpoint/model/Key 语义，不恢复 +204 将 SDK 参数名 `extra_body` 直接序列化到 JSON 的错误；Gemini thinking 配置必须按真实线端协议发送并继续解析服务实际返回的 `reasoning_content`。
4. 当前 +205 的提供商选择是全局开关：`SecureConfig.readApiKey/readEndpoint` 被普通主回复、沉浸正文、NSFW 路由、Agent、经验整合、阶段摘要、AI Self、翻译和自主联网协调共同读取。因而选择玩游会让机械工作也逐次扣 0.5，不能用简单“换回 +204”完成；下一版必须引入明确的 internal worker lane 与 final expression lane。
5. 冻结的第一版分流：NSFW/场景路由、Agent 工具规划、结构化 JSON 抽取、每轮 Memory/Relationship/Desire 整合、人格学习语义复核、阶段摘要、AI Self、外语投影、思考翻译与其他后台机械任务使用 DeepSeek；用户触发的普通聊天最终正文和沉浸小说最终正文，在用户选择 Gemini 时使用玩游。工具只由 DeepSeek 选择并在手机端执行，Gemini 只接收真实 Outcome 与完整人格/世界书/记忆/历史 Prompt 后写最终话语，不得重做工具或宣称未发生操作。主动消息第一版默认 DeepSeek，是否另设“跟随回复模型”开关待正式开始时确认，避免默认在无人操作时持续扣固定积分。
6. 成本目标不是保证所有回合永远只有一次请求，而是正常无重试的可见回合只消耗一次玩游请求，即 0.5 积分/¥0.02；内部 DeepSeek 继续按 token 计费。沉浸被截断后的续写、真实网络重试或最终事实声明修正可能增加玩游请求，必须在实现时做有界处理并记录。第一版不依据请求前的粗糙 token 估算频繁跨模型切换；先按职责稳定分流并记录每条 lane 的调用次数/usage，后续若证据表明 Memory/长摘要等少数机械请求在 DeepSeek 上反而超过 ¥0.02，再单独设计成本路由。
7. 用户截图 `1000153429.jpg` 显示 +205 沉浸房间 AppBar 的中/日/EN 三个按钮纵向占满顶栏，明显不同于预期紧凑胶囊。源码原因范围已定位：普通聊天同款 `_LanguageSpeechButton` 本身没有异常；沉浸把 `_languageSelector()` 直接放进 `AppBar.actions`，而 `_ImmersiveLanguageButton` 只有 `minHeight:24`、没有外层固定高度/居中约束，因此受 AppBar action 约束拉高。下一版使用明确紧凑高度与居中包裹，并加 widget 尺寸断言；不改变三语选择、缓存或 TTS 语义。
8. 同一截图中的多段动作/神态被 Gemini/DeepSeek 输出为全角 `（……）`。当前有效 07 沉浸规则已经明确“不使用普通聊天的动作括号”“叙述不加括号”，不存在遗漏提示；因此按用户决定先记为模型偶发格式不服从，下一版不重复堆禁令、不做字符串剥括号或渲染层伪修复。只有后续稳定复现并证明与提供商/Prompt 层冲突有关时再窄修。
9. 用户确认 Gemini 本身存在偶发服务不稳和正文中断，决定 Gemini 每个最终回合最多尝试 2 次。第 2 次只给 timeout、连接失败、未收到正文即断流、429、500/502/503/504 等瞬时错误；400/401/403、模型名、参数和协议错误不浪费第二个固定 0.5 积分，立即切 DeepSeek。两次均失败后由 DeepSeek用同一完整 Prompt 完成本轮，只允许落一条 assistant 与一份 post-turn；普通聊天和沉浸均显示不会自动消失、可手动关闭的“Gemini 失败，本轮已由 DeepSeek 兜底”提示。正常 Gemini 成本 0.5 积分；发生一次瞬时重试时 Gemini 成本最多 1.0 积分，DeepSeek 兜底另按 token 计费。
10. 用户进一步明确“有正文的异常断流”不等同于无正文调用失败：部分内容有时已经足够，应让用户选择，而不是每次强制重跑。冻结 UI 为两个操作：刷新图标位于 TTS 喇叭左侧并先弹“重新生成这条回复？”；另有“确认回复”按钮并二次确认。重新生成前草稿不得进入普通历史 Prompt、Memory/post-turn 或沉浸原文/滚动摘要；确认后才按正常消息原子提交并进入后续整理。普通聊天复用 `generation_jobs.partial_*` 并新增非自动恢复状态 `awaiting_confirmation`；沉浸使用本地 settings 中的房间/用户/正文草稿，不插入 `immersive_messages`，确认后才写入。应用重启仍应恢复待确认草稿。
11. 已完成的任意历史回复刷新与上述“未提交截断草稿”严格分开：普通完整回复提交后会立即触发 Memory、Relationship、Desire、人格学习、Thought/thread 与 Somatic 等多类副作用，当前没有统一逆事务；只替换 `messages` 会产生不可见旧记忆污染。本批不得开放完整历史刷新。后续若实现，必须先设计 assistant revision/supersede 身份、post-turn provenance 与逐模块可撤销/重算事务。
12. TTS 排查找到可证实的项目缺口：DeepSeek 外语投影只校验 JSON 段数/非空，不校验目标语言，导致模型偶发把日文或中文混入英语投影后仍持久缓存，TTS 会忠实朗读错误缓存。本批为英语投影增加 CJK/平假名/片假名污染校验，失败后用 DeepSeek 最多重试一次；旧污染英语缓存也不再复用。中文与日语可合法含汉字，因此不做粗暴删字；若纯中文正确文本在小酒狐日语原声模型上仍偶发呈现日语口音或拟音，只记为声学模型边界，不伪称可由 Prompt 修复。
13. 已从 +205 head 开出 `agent/v04162-hybrid-final-fallback`，目标 `0.41.62+206 / schema 60 / Snapshot protocol 6`。当前实现已把 legacy `SecureConfig.readApiKey/readEndpoint` 固定回 DeepSeek internal lane，并新增 final-reply lane；设置页始终要求 DeepSeek，选 Gemini 时才额外显示玩游 Key/固定 endpoint/model。普通与沉浸最终请求正在接入有界重试、DeepSeek 兜底、持久提示和安全草稿；AppBar 三语按钮改为外层 28 高、按钮 32×24。尚未完成 Flutter 编译/测试、CI、APK 或真机验证，不能提前提升状态。
...
18. 用户提供 +206 当日原生备份与脱敏诊断并明确反馈“重要的真机测试都通过了”。备份 ZIP 完整，Snapshot protocol 6 / schema 60，`state.json` 19,167,204 bytes，实算 SHA-256 `f5db92376707dec4d374e511906f1dd476eb81aff9c5b019fcfb73da45d58320` 与 manifest 一致，媒体清单无缺失。诊断为 `0.41.62+206`；数据库可开、Active Brain、后台 Dart 通道、悬浮触摸/安全位置、系统页面返回后自愈、Accessibility 授权/连接/事件心跳、通知投递与确认、Android 后台条件、音频路由、公网发现和 TTS 核心资源均为 pass；实际运行有 `audio_playback → audio_complete`。`generation_jobs` 为 515 completed / 15 cancelled_by_user / 5 interrupted，当前无 active、failed 或 `awaiting_confirmation`，也无沉浸草稿。因本次未触发 Gemini 故障或异常断流，报告不能单独证明两次重试、持久兜底提示或草稿两选按钮；这些保留为专用故障分支待触发，不影响用户对已实际验收主链的通过结论。诊断为非 deep 导出，故 TTS `integrity=unchecked / artifactCount=0` 只表示未执行黄金哈希深检，不是资源失败。
19. 用户随后明确更正此前口述，以本条三项有序矩阵为唯一有效真机事实：①首次用中文生成/播放后，切中文或英语均能正确播放，切日语不能播放；②首次用英语生成/播放后，切中文或英语均能正确播放，切日语不能播放；③首次用日语生成/播放后，再切中文、英语或日语均能正确播放。故此前依据最后一条纯中文消息推断“中文前端把中文稳定读成日语”属于证据越界，现已撤销；那条消息和全局 `tts_language=zh` 只能证明中文文本/选择没有被日语缓存替换，不能证明用户实际听到的失败发生在中文前端。
20. 修正后的矩阵与脱敏诊断互相吻合：报告中三次真实失败均为 `generation_failed / NullPointerException / language=ja`，而不是 `language=zh`；备份同时存在多条已持久化日语 variant，说明至少部分失败发生在外语正文已经生成之后，不是 DeepSeek 没给日语文本。源码当前每次生成都会按目标语言调用 `initialize(language)`，切换时卸载声学模型并 `releaseFrontend()`，随后只构造目标前端；按钮本身不预热，首次播放会惰性准备对应前端。因此“选择中文时禁掉日/英音素”在逻辑上已经成立，不能解决此缺陷。最强候选变为顺序相关的日语原生前端/共享声学 runtime 生命周期：fresh→日语可成功，而中文/英语先装载声学 session 后再释放并切入 `NativeJapaneseFrontend` 时可能留下失效状态或触发内存/原生空引用。+207 应以 fresh→zh→ja、fresh→en→ja、fresh→ja、ja→zh/en→ja 自动与真机矩阵复现，并只在“从非日语进入日语”边界尝试完整重建隔离 runtime；同时把阶段诊断细化为 frontend prepare、acoustic load、infer，不用 Prompt、翻译重试或禁播掩盖原生错误。
21. 用户明确“三语万能播放”是期望能力：任意历史消息都应能按当前选择读中文、日语或英语；当初考虑 CN/JP/EN 标记只因担心禁用另外两语后无法知道消息首次语言，若万能播放成立则不需要标记。源码证明无需同时预热三语：每个 TTS session 把 `language` 固定到整条队列，`prepareLanguage` 会先卸载旧声学 session、释放旧前端，再只构造目标前端；缺少日/英文本时由 DeepSeek 按需生成并缓存后再播放。因此万能播放与“单一前端常驻”完全兼容，不会让三套前端同时拖慢每段推理。代价仅是切换语言时的冷准备/声学模型重载延迟；可在用户发送新一轮且语言已选定时，与云端 LLM 生成并行准备该单一前端以隐藏延迟，历史消息保持点击后惰性切换。快速连点必须以 generation token 丢弃过期准备，不能出现旧语言晚完成后覆盖新选择。
22. 沉浸房间当前确实等完整 LLM 回复、续写判定、消息提交以及必要的日/英全文投影全部完成后，才调用 `playText`。底层 `TtsPlaybackQueue` 已具备 `beginStream/addDelta/endStream`，但生产 controller 尚未接入。低延迟优化可先只对中文启用：按 `tts_reading_scope` 在流中缓存到闭合句/闭合 `「」` 后立即入队，未闭合尾部不读；停止或异常断流时立即丢弃尚未播放队列。日语/英语目前依赖完整中文后的单次投影，为避免多次翻译扣费和前后文不一致，继续在全文完成后朗读。此项与日语 runtime 缺陷可同一规划但须独立测试和归因，不应把流式优化当作日语修复。
23. 本次备份 ZIP 为 35,969,529 bytes（约 34.3 MiB），解压 51,760,824 bytes；其中 30 个媒体文件占 32,588,175 bytes，15 个 `media_blobs` 均有真实引用、无 orphan，消息/相册复用同一 blob，没有发现同图重复拷贝。原图约 24.6 MB，缩略图约 8.0 MB；PNG 缩略图压缩效率偏低，可后续改为更小尺寸/WebP 或 JPEG，但不属于数据重复。`state.json` 解压 19,167,204 bytes，主要增长来自 `desire_events` 22,493 行、`device_events` 15,330 行、消息与感知。进一步源码复核纠正“Desire 无限保留”的初判：`recordDesireEvents` 每次写入都会删除 14 天以前的行，但缺少最大行数 cap，故高频 14 天窗口仍可很大；它只供 24 小时脱敏诊断统计，当前欲望与基线权威状态在 `desire_state`，不参与 Memory/Prompt 回读。`device_events` 已由长期维护按 21 天且最多 12,000 行裁剪，但最近 30 分钟信号、12 小时屏幕状态及上次感知后的新事件会参与设备情境解释并可间接形成短期 Awareness/Desire，因此不能完全停存或随意全删。后续可将两表保留为短期本地运行记录、从跨设备备份中导出为空：Desire 历史改为 24～48 小时且另加行数 cap；Device 历史保留当前判断所需的最近 12 小时并继续 cap。设备与数据页增加带确认的“清理旧运行记录”，明确不删除 `desire_state`、Thought、Memory、关系、聊天或当前 Awareness；若提供全清设备事件，须提示短时间内当前情境会重新积累。
24. 设置导航审计确认用户所说的重复结构真实存在，但当前源码中的具体位置与口述方向相反：侧栏“全部设置”打开 `/settings` 的七域新页，其中已经有“设备与数据”；底部“更多”仍是旧五域 `她/你们/能力/手机感知/数据与高级`，没有直接的“设备与数据”，只在“能力”中绕回 `/settings`。各具体设置页与 setting key 是共享的，没有两套配置状态，重复的是两套导航中心。后续应以底部“更多”为唯一总入口：先逐项把 `/settings` 的模型与联网、记忆与成长、主动与感知、语音呈现、表情包、设备与数据、诊断与开发映射过去，并保留旧“她/你们”、查手机、沉浸房间、状态、世界书和素材说明的可达路径；覆盖测试证明无独有入口丢失后，侧栏删除四个分类设置快捷项，只留一个“全部设置”直接切到底部“更多”。不能先删除 `/settings` 或页面类，具体页面应继续复用，避免重新实现和保存语义分叉。
...
21. 用户在动作发生前再次确认后，AI 伴侣私有 Draft `genie-tts-private-runtime-v0.7.6-jiuhu` 已保存为未发布状态，Release 列表显示 `Draft / Assets 1`，附件名正确。公开远端 build commit `543d7be191a56c69d04d8d355d80dff462201765` 的 tree `b62ce0106785d171f51db91c0339b8fd73237ade` 与本地 +204 HEAD tree 精确一致；run [`34559566810`](https://github.com/catkiss62/ai-companion-build/actions/runs/34559566810)（828）在首个私有载荷恢复步骤失败，日志先确认 APK SHA 为 OK，随后 `unzip` 报找不到 `assets/openjtalk/*` 与 `libgenie_frontend.so`。本地逐项列出原 APK证明它确实只含 `assets/benchmark_jiuhu`、`libopenjtalk_native.so`、`libc++_shared.so` 与 ORT 库；这不是下载截断，也不是小酒狐两个源模型混淆。窄修方案为：小酒狐模型/候选继续只从 v0.7.6 精确 APK恢复；OpenJTalk 字典与 JNI adapter 整套从 +203 已完整构建验证、固定 SHA 的 v0.6.4 私有 APK恢复，且绝不解出其中 `assets/benchmark/*`。修复前状态严格为 `CI FAILED EARLY`，不能写成编译或测试通过。
22. 载荷拆源修复由远端 `e4e15e179fd009e391a8249aa3dde25a8d351a07` / tree `74688cbc454547fbea8449de1fa590593473d857` 触发 run [`34559907462`](https://github.com/catkiss62/ai-companion-build/actions/runs/34559907462)（829）。该 run 已证明小酒狐与共用前端两个私有 APK下载/固定 SHA、严格分目录解压、干净生产基线、417 桌宠、LingChat、塔罗、Java/Flutter 工具链和私有签名均可用；随后 Source validation 在 v0.41.55 历史合同处失败：`ReasoningEffort.low` 的 UI label 被本批改成中文“低”，破坏既有 `Low` 文案合同，且 v0.41.57/58 仍要求工作流保留“six prompt tensors and per-file integrity metadata”历史说明。修复保留已有 Low/High/Max，只为 Gemini 增加 Medium，并补回纯注释 token；不改 API 字段、思考级别、模型或 TTS。按工作流相同顺序本地重跑 83 个 validator 为 80 passed / 3 environment-only unavailable，三项仅因工作区没有 CI 恢复的 417 桌宠、LingChat effects 与 `kotlinc`；Gemini、小酒狐、总账、workflow YAML 与 `git diff --check` 均通过。待推送第三轮真实编译。
23. 历史合同窄修由远端 `fec6a0390abec3fac7cd87abe3fb76c59b7c10a3` / tree `afbc2d2260197a095d34fe458419a672a0bdb01b` 触发 run [`34560293101`](https://github.com/catkiss62/ai-companion-build/actions/runs/34560293101)（830）。该 run 已通过两套私有载荷恢复与 SHA、干净生产基线、全部源码/历史 validator、Flutter 依赖、Kotlin/JVM tests 与 Flutter Analyze；全量 Flutter tests 为 716 passed / 1 failed。唯一失败是 `chat_api_provider_test.dart` 的 SSE 假响应含中文“先计算。”，但 `http.Response(String)` 在未声明 charset 时按 Latin-1 编码并在 MockClient 内抛出 `Invalid argument (string): Contains invalid characters`；堆栈停在测试第 51 行响应构造，尚未进入 `DeepSeekClient` 的 SSE/`reasoning_content` 解析。这是测试夹具编码错误，不是 Gemini 不能 thinking、响应协议错误或生产思考摘要丢失。预定窄修只给该假响应添加 `charset=utf-8`（或等价 UTF-8 bytes），保留中文摘要以真实覆盖 Unicode 流式路径；生产代码、TTS、模型资源与请求字段不改。
24. UTF-8 窄修已在 `chat_api_provider_test.dart` 完成：MockClient 的 SSE 响应 `content-type` 从 `text/event-stream` 改为 `text/event-stream; charset=utf-8`，测试正文、中文 `reasoning_content`、Gemini 请求断言和生产代码均未改。这样 `http.Response` 会按 UTF-8 编码测试流，下一轮既能越过测试夹具构造，也仍会验证现有解析器把 Gemini 中文思考摘要与正文分别交付。本地已通过 `git diff --check`、+204 Gemini/小酒狐专项、当前总账 archive/hash/50KB 门、workflow YAML 与 Python 语法；因环境无 Flutter SDK，真实单测结果仍须 Actions，不能在推送前伪称已通过。
25. UTF-8 窄修通过远端 `401f2aa2b11d9b074a4b73d039eef45cd772633a` / tree `b944fdeb68c4a1e5b8447bb519fb048bd45e23cd` 触发 run [`34561182823`](https://github.com/catkiss62/ai-companion-build/actions/runs/34561182823)（831）。该 run 已通过两私有载荷固定 SHA、干净生产基线、全部源码/历史 validator、Kotlin/JVM、Flutter Analyze、全部 Flutter tests、Release APK 编译与固定签名，证明中文 Gemini `reasoning_content` 流式测试已通过。最终“Verify release APK Genie”先确认 OpenJTalk ELF 依赖闭包完整，随后在准备比较应用内 `libonnxruntime.so` 与私有 Genie 原件时，对 `.genie-payload/unpacked/lib/arm64-v8a/libonnxruntime.so` 抛出 `FileNotFoundError`；首步只从 v0.7.6 APK 解了 `assets/benchmark_jiuhu/*`，没有为后置验证解出该已存在的 native 库。失败发生在读取对照源之前，尚未得出字节不一致，也不是 APK 缺库。预定窄修只在 v0.7.6 解包白名单增加 `lib/arm64-v8a/libonnxruntime.so`，继续让应用自己的依赖参与打包，再在实包门做 byte-for-byte 比较；不得从旧 v0.6.4 前端 APK取 ORT，也不得覆盖或改变运行路径。
26. ORT 对照输入窄修已完成：v0.7.6 私有 APK 解包白名单增加 `lib/arm64-v8a/libonnxruntime.so`，但后续复制到 `app/android/app/src/main/jniLibs` 的列表不增加它，因此应用仍由既有 ONNX Runtime 依赖供应最终库；临时原件只供 step 18 byte-for-byte 比较。已直接从本地重组且整包 SHA 已验证的 v0.7.6 APK检查该条目：18,214,224 bytes、SHA-256 `3fc3e79539f06f67360a810e60eb5c513db5febc16f7bc71e5d9e8ce39e3d775`。+204 validator 新增唯一说明 token锁定这条“只做对照、不覆盖应用”的边界；`git diff --check`、专项、当前总账、workflow YAML 与 Python 语法均通过。运行代码、四候选、Gemini 请求/思考摘要、Flutter test 及旧前端拆源均未修改。
27. ORT 对照窄修由远端 `28c7c11a079017747c571a2fe35b06f6a65b0095` / tree `0af4358b36612938a10f875f3cfb38a137b5c3eb` 触发 run [`34562176440`](https://github.com/catkiss62/ai-companion-build/actions/runs/34562176440)（832），该 run 完整成功。`717/717` Flutter tests、Kotlin/JVM、Analyze、Release 与固定 signer 均通过；实包门确认 OpenJTalk ELF 闭包完整、应用 ORT 与 v0.7.6 Genie 原件 byte-identical、48 项 Genie/Jiuhu/OpenJTalk 资源大小/SHA 精确、四个小酒狐候选及六张量齐全且彼此独立，恬豆、奶油、Meju、测试备份和用户 RoBERTa 均不存在。最终 APK `AI-Companion-v0.41.60-204-Jiuhu-Gemini-Relay-APK.apk` 为 544,651,810 bytes、SHA-256 `a4da80538999ae8be38d2dd18b584ca0d9ed48b29c5e91082a3821ed93ae5464`，与 Draft asset digest 一致。Artifact `10184946079` 为 537,772,837-byte ZIP / digest `4b70f398a3094628dbd595ca777acbe09807523a97f8c5c51fc34e49a1fa7a0c`；APK、checksum 与 CI monitor 已上传未发布 Draft `v0.41.60-jiuhu-gemini-relay-test`。`main` 未合并、Release 未发布。自动化只能提升为 `APK READY`；小酒狐实际听感/自动音色、默认音高与真实玩游 Key/思考摘要仍为 `TRUE DEVICE PENDING`。
...
11. 本地实现提交为 `60eb5a1` / tree `76464f7f0503d5e3fb7b32a2df7e0f4a50c34628`，共 48 个任务相关源码、测试、validator、workflow 与总账文件；不含模型、参考音频、RoBERTa、用户数据、备份、诊断或密钥。首次普通 `git push` 被发布审查要求本轮再次明确确认；用户随后明确同意推送到 `catkiss62/ai-companion-build` 的 `agent/v04159-tiandou-pitch-recovery` 并运行 Actions。容器没有 Git 凭据，故改用会话已授权的 GitHub Git Data 接口按本地 tree 原样创建远端实现提交 `6597b3528d6d3ff09bba848b3550a1b5914b2b69`；其 tree 与本地实现 tree 完全一致。最终总账提交创建后只将指定开发分支 fast-forward 到该提交，不合并 `main`、不发布正式 Release。
12. 首次创建远端分支后没有 Actions run；复核确认 workflow 的 push 分支白名单只列到 `agent/v04155-genie-direct-port-lazy-language`。已补入当前 `agent/v04159-tiandou-pitch-recovery` 并由 +203 validator 锁定，远端构建接线提交为 `381f7c75fed1d53eac8f83cfcbc68b5a50d6ac1c`；该提交随后成功触发唯一一次完整构建。最终总账只以 `[skip ci]` 文档提交同步，不重复生成 APK。
13. Actions run [`34508603932`](https://github.com/catkiss62/ai-companion-build/actions/runs/34508603932)（827）完整成功。私有 v0.6.4 恬豆恢复和干净基线先通过；源码/回归 validator、新增 `AssetIntegrityPolicyTest`、Kotlin/AIDL、Flutter analyze 与 `715/715` Flutter tests 全绿；随后 arm64 Release、固定签名、40 项 Genie/Tiandou/OpenJTalk 实包大小/SHA、完整 306,827,268-byte T2S、`ref01/ref02/ref04/ref06`、无奶油/候选5/Meju、ELF 依赖闭包和 Draft 上传全部通过。签名 SHA-256 保持 `30:5E:B3:D8:09:83:B9:63:C6:48:18:DD:F1:AD:56:1F:27:9D:E6:D4:7B:3E:D2:C7:81:AD:A4:48:C7:C2:51:48`。
14. 最终 APK `AI-Companion-v0.41.59-203-Tiandou-Pitch-Recovery-APK.apk` 为 537,638,144 bytes，SHA-256 `a3c545caf08ef0c3edfa589e61642d92ea1b26421f1ac9528ffa59aa010827fd`，与 Draft 服务端 digest 一致。Artifact [`10165300992`](https://github.com/catkiss62/ai-companion-build/actions/runs/34508603932/artifacts/10165300992) 为 530,779,822-byte ZIP / digest `134cf4b596d3fdbbe7d7bc427c96a0b4429c07a8bb5d07457d0263df6a14f97f`；成品位于未发布 [Draft Release](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-f05cdbac5b237ee9200e)，`main` 未合并。状态提升为 `CI PASSED / APK READY / TRUE DEVICE PENDING`；下一步只按顶部清单做真机听感与三语抽查。
...
7. 完整实现通过 Git Data 接口以远端 +201 总账 head 为父创建 build commit `3ab7ecd3667e243a2aac325dd742e07d8ce3fde3` / tree `d77e4a33c17886f7fd4b3d46ac5fb5ceb1abb83e`。首轮 Actions run [`34469916944`](https://github.com/catkiss62/ai-companion-build/actions/runs/34469916944)（825）通过资源、源码、Kotlin/AIDL、Analyze 和全部新增 TTS/备份测试，完整 Flutter tests 为 712 passed / 1 failed；唯一失败是旧测试仍期待 +201 的 36%/55%，运行代码已正确回退 24%/42%。只修正该过期断言后创建 `bd886d304e3d94b32ed98c6c009080c990282251` / tree `a348a7ddf505959c43cad4617e2fd47492611f14`，没有回滚新逻辑。
8. 第二轮 Actions run [`34470775012`](https://github.com/catkiss62/ai-companion-build/actions/runs/34470775012)（826）完整成功：全部源码/回归 validator、Kotlin/AIDL、Flutter analyze、`713/713` Flutter tests、arm64 Release APK、固定测试签名与 OpenJTalk ELF 依赖闭包均通过。实包校验确认 40 项 Genie/Naiyou/OpenJTalk 文件大小与 SHA 精确；生产 manifest 只含 `naiyou_growth/naiyou_hello/naiyou_dog/naiyou_dynamic`，候选5、恬豆与 Meju 不存在。签名 SHA-256 继续为 `30:5E:B3:D8:09:83:B9:63:C6:48:18:DD:F1:AD:56:1F:27:9D:E6:D4:7B:3E:D2:C7:81:AD:A4:48:C7:C2:51:48`。
9. 最终 APK `AI-Companion-v0.41.58-202-Backup-Naiyou-Hotfix-APK.apk` 为 535,444,192 bytes、SHA-256 `e2d80ff3ae65c37adc3a2edeb233c4d0e0a0906188b0644573b64e8ca5cb092c`，与 Draft 资产服务端 digest 一致。Artifact [`10149802145`](https://github.com/catkiss62/ai-companion-build/actions/runs/34470775012/artifacts/10149802145) 为 528,578,840-byte ZIP / digest `541af3bafaebf02a7e908303b0ed6d93952dae090e43bdc4102cef620f52a6a9`；成品位于未发布 [Draft Release](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-e75e3ac276ff5f165a4d)，`main` 未合并。状态提升为 `CI PASSED / APK READY / TRUE DEVICE PENDING`；自动化不能代替真实备份、日语发声和声音听感验收。
...
3. 表情“自然”档当前条件概率为 24%，但此前还有 casual、6 类 speech act、100 字、情绪 mood、近 18 张去重、bold 限制和正语义分数等门；语义又只匹配助手短回复，真实命中率会远低于 24%。本轮将自然档提高到 36%，并把“用户本轮文本 + 助手回复”共同作为语义匹配上下文；仍禁止零分随机图、任务/事实回复乱配图和短期重复。`shouldUseStickerOnly` 的 30% 只决定已选中的表情是否替代短文字，不是发表情的概率。
4. 用户本轮附件 `自然化.txt` 为 2,242 bytes / SHA-256 `13189a1fcb24f10eb071f455356ffd902d3eafa1abb8466a28580b768bf9a393`，只包含从“# 自然对话总则”到“# 对话自然度优先级”的中文正文；它与 +200 默认前面含“角色思考方式真人化”的版本不同。本轮按附件逐字替换，不提交附件；使用新的幂等 marker 和旧默认 SHA 白名单迁移 +200 已知正文，未知用户手改继续优先。
5. 目标版本为 `0.41.57+201 / schema 58 / Snapshot protocol 6`。本批没有表结构变化，因此不虚增 schema；不改共享媒体表结构、Memory/Thought/Desire、Agent Outcome 或 `main`。本地通过后按持续授权推送当前开发分支并触发 Draft 测试 APK，CI 与真机状态严格分开。
6. 用户在实现中途追加已完成独立真机试听的奶油语音替换，并再次纠正映射：奶油候选 3→日常、2→温柔、4→活泼、1→可爱；候选 1/2/3/4 全部需要，只有候选 5 删除。恬豆不得与奶油共存，以免保留双模型。奶油为旧 322 音素 GPT-SoVITS V2，测试项目的构建期 ONNX 元数据/频谱 Slice 修复已经完成；伴侣继续复用现有中文 Chinese RoBERTa、英语 ARPAbet 与日语 OpenJTalk 前端，只替换声学权重及四个参考条件。+199 已通过的语言切换、按需单外语、停止、语速/音量和播放不拉底行为均为不得回归项。
7. +201 本地实现已完成且前四项没有因 TTS 插入而丢失：设置入口/页面/扫描与确认按钮均直写“重复图片与表情包/旧重复副本/exact-SHA”；普通聊天把新 user commit 作为独立时间线落底事件，TTS-only 仍无滚动请求；沉浸和悬浮已有发送落底只加静态合同；表情自然档为 36%、较多为 55%，以用户本轮文本与助手回复共同做正语义匹配，仍禁止零分随机图；自然化正文为 2,242 bytes、SHA-256 `13189a1fcb24f10eb071f455356ffd902d3eafa1abb8466a28580b768bf9a393`，只迁移已知默认 SHA。
8. 奶油生产映射已经写入 `GenieTtsRuntime` 与 `VoiceProfileCatalog`：`naiyou_growth`（候选3）→`daily`，`naiyou_hello`（候选2）→`gentle`，`naiyou_dog`（候选4）→`lively`，`naiyou_dynamic`（候选1）→`cute`。中文、ARPAbet 英语和 OpenJTalk 日语仍进入同一 322 音素声学模型；按需单语言前端、独立 `:genie_tts` 进程、串行 ORT 所有权、停止与单 AudioTrack 播放合同未改。运行资源 marker 已换为 build 201，避免覆盖安装继续误用旧恬豆解包目录。
9. 私有构建资源已保存到 `catkiss62/ai-companion-build` 的未发布 Draft release `386129339`，tag 为 `genie-tts-private-runtime-v0.7.1-naiyou`；`Genie-TTS-v0.7.1-Naiyou-Runtime.zip` 为 319,200,434 bytes、SHA-256 `ecfb0bc90f9c29b7d9be53bd3bb5f952e7bef9486f4fa78140e3c502aa4a6423`，GitHub 端 digest 一致。公开 Git tree 不含奶油权重、参考音频、RoBERTa 或 APK。该私有源包保留测试项目的五条原始候选供可复现裁剪；Actions 在写入生产 APK 前只保留 1/2/3/4 并删除候选5、全部恬豆 case、原始参考 WAV 与外置 RoBERTa，最终实包门再次拒绝任何残留。
10. 本地已用与 Actions 相同的裁剪脚本模拟私有载荷：生产 manifest 的 character 为 `naiyou`，case 精确为 `naiyou_dynamic/naiyou_hello/naiyou_growth/naiyou_dog`，共 28 个奶油文件、491,237,150 bytes；候选5与恬豆 token 均不存在。`git diff --check`、workflow YAML、v0.41.55/56/57 专项、总账校验及 Actions 列表 80 个 validator 中 77 个均通过；剩余 3 个只因本地没有 CI 恢复的 417 桌宠包、LingChat effects 与 `kotlinc`。本地 Gradle 又因受限网络无法下载 Gradle 8.12，Kotlin 编译、Flutter analyze/全量 tests、Release/签名和最终 APK 载荷严格等待 Actions，不能记为已通过。
11. +201 完整实现通过 Git Data 接口以远端 +200 总账 head 为父提交创建 build commit `ce0eaa45ae311a6aa4daf81f5d96591f2cc4af83`，tree `3bfe2836e19abdf67e95fdc2c6532a08a3a92fdf` 与本地实现 tree 逐字节一致。公开提交只含任务源码、测试、workflow 与总账开工记录；不含用户附件、诊断、备份、密钥、奶油/恬豆模型权重、参考音频、RoBERTa 或 APK。
12. Actions run [`34458743296`](https://github.com/catkiss62/ai-companion-build/actions/runs/34458743296)（824）完整成功：全部源码/回归 validator、Kotlin/AIDL、Flutter analyze、`707/707` Flutter tests、arm64 Release APK、固定测试签名和 OpenJTalk ELF 依赖闭包均通过。实包校验确认 40 项 Genie/Naiyou/OpenJTalk 文件大小与哈希精确；生产 manifest 只含 `naiyou_growth/naiyou_hello/naiyou_dog/naiyou_dynamic` 四个 case，候选5、全部恬豆、Meju 与外置 RoBERTa 均不存在。签名 SHA-256 保持 `30:5E:B3:D8:09:83:B9:63:C6:48:18:DD:F1:AD:56:1F:27:9D:E6:D4:7B:3E:D2:C7:81:AD:A4:48:C7:C2:51:48`。
13. 最终 APK `AI-Companion-v0.41.57-201-Chat-Media-Naiyou-APK.apk` 为 535,443,780 bytes、SHA-256 `c3567fc8c9a28c6c44ba17a19a3a8fe395b2ff07f83a83ed5bfba31c0d7e580f`，与 Draft 资产服务端 digest 一致。Artifact [`10145057720`](https://github.com/catkiss62/ai-companion-build/actions/runs/34458743296/artifacts/10145057720) 为 528,578,717-byte ZIP / digest `0190c6f2721762255a81f4b25b2cf6c6283b480dae33ca9c9245e415b83d783f`；成品已保存到未发布 [Draft Release](https://github.com/catkiss62/ai-companion-build/releases/tag/untagged-29928c3f374ed1b9997e)，`main` 未合并。状态提升为 `CI PASSED / APK READY / TRUE DEVICE PENDING`；下一步仅按顶部清单做六项合包真机验收，自动化不得代替声音体感、主动表情相关性和界面行为验收。
...
3. 私有附件 `AI_Companion_Backup_2026-09-10T03-04-12.aibackup` 只在本地读取目标世界书字段，不提交聊天、附件、诊断、备份或任何私人媒体。已核对：日常对话规则 691 字、priority 900、scope `chat|proactive`；造梗能力 2,702 字、存档当时 probability 10、priority 650；角色表达自然化为用户新增中文 2,603 字、priority 1000、scope all；推演思维引擎 3,390 字、priority 950、scope immersive。
4. 按用户最后决定，造梗不采用两层概率：世界书本身使用唯一 30% 注入概率；正文中的 15%～30% 只表达命中且适合时在本轮回复中的篇幅，不再作为第二次触发。命中后是否使用及使用哪种造法交给模型按语境判断。`DialogueExpressionPlan` 仍可保留与幽默无关的任务/反馈/深入回应重心，但必须删除独立幽默关键词、阈值、造法抽签、第二造法与“本轮强制写进正文”注入；关闭或未命中造梗世界书时不得再由该旁路暗示谐音变异、接梗或加码。
5. 三项非造梗世界书正文必须逐字符采用存档版本，不擅自润色；造梗只允许两处已授权差异：此前要求移除的年龄/身份边界措辞，以及本轮单层概率语义。默认和旧安装迁移均使用精确正文/hash 白名单，任何不命中已知版本的用户手改继续保留。
6. 年龄边界继续按 +197 的有效注入链规则清查：所有发送给聊天 LLM 的默认规则、普通/主动/沉浸 Prompt、世界书、学习与人格模板中不得重新出现“成人/未成年/年龄模糊”等边界。历史识别哈希、非 Prompt 的视觉分类元数据或数据库兼容常量可保留，但不得进入有效 Prompt。
7. D6 冻结合同不变：旧 `.aibackup` 永不改写；设置页“优化媒体存储”先 dry-run 显示 exact-SHA 可合并数量与空间，用户确认后才迁移运行库，且可重复执行。新媒体进入 content-addressed blob；message、album 与 sticker 来源只持有独立引用/引用计数。保存到相册只增加同一 blob 的 album ref；缓存页排除仍有永久相册引用的媒体，支持多选、全选、合计可释放空间；删除一处引用不能破坏其他消息/相册引用。
8. D6 迁移只按可信 provenance + 原图 exact SHA 合并；不重跑识图，不用感知哈希做破坏性去重。Snapshot protocol 6 导出共享 blob 与引用关系；恢复 protocol 5 后旧路径仍可显示并可再次手动优化；跨设备且没有原表情包时，历史消息仍必须带可恢复的 blob，不能依赖外部图库包。
9. 预定验证分三组：A）四项世界书精确正文、priority/scope、单层30%、迁移保守性和有效 Prompt 无年龄门/无硬造梗卡；B）新发普通图片、双方表情、相册保存的 exact-SHA 复用与引用安全；C）dry-run/确认/二次幂等、缓存多选删除、旧 protocol 5 导入、protocol 6 导出恢复及 orphan 清理。Actions 还须完成 Flutter analyze/全量 tests、Kotlin/AIDL、arm64 Release、固定签名与所有私有载荷门；CI 通过仍不等于真机通过。
10. 修改前双层总账已单独提交为 `eab80aa`，锁定 +199 真机基线、+200 范围、世界书精确来源、单层概率语义、年龄边界与 D6 数据安全合同。实现未读取或改写旧备份内部媒体，私有 `.aibackup`、聊天、诊断、图片、表情包、密钥和 Genie 私有载荷均未加入 Git。
11. 世界书本地实现已完成：新增 v0.41.56 用户正文源；“角色表达自然化”2,603 字和“推演思维引擎”3,390 字逐字符匹配存档 SHA，默认分别为 `1000/all` 与 `950/immersive`；“日常对话规则”为 `900/chat|proactive`；“造梗能力”为 `650/30/chat|proactive`。造梗正文只改已授权的年龄身份例句和单层概率说明；迁移只对白名单 SHA/精确旧正文生效，未知用户编辑保持不动。
12. 造梗硬旁路已移除：`DialogueExpressionPlan` 只保留 casual/feedback/challenge/deep/task/sensitive 的本轮回应责任分类，不再包含 humor enum、关键词命中、阈值、turn hash、主/副造法、强度或“必须写进正文”的执行卡；`PromptBuilder` 不再计算 subjective playfulness 来独立触发幽默。旧诊断字段只兼容解码并恒记 `none`，不会重新影响 Prompt。唯一触发层是世界书 30% 注入；正文 15%～30% 只是适合使用时的回复篇幅参考，模型仍可判断不用。
13. D6 本地实现已完成：schema 58 新增 `media_blobs` 及 message/album `blob_id`，原图 SHA 为稳定 blob 身份，缩略图另校验 SHA；新聊天图片、用户/助手表情与联网相册图均写入共享内容寻址目录。聊天图片保存相册只增加 album ref，不复制字节；删除消息、相册软删除到期和缓存批量删除均先更新独立引用，再只回收真正无所有者的 blob。
...
5. TTS 开工时保护：不改 schema、Memory/Desire/人格、中文历史真源、四音色映射或 Genie 固定核心；不把 GPU/NNAPI 实验与当前真机修复混包。用户随后明确把规则 05/06/07、造梗默认、年龄边界清理和沉浸引号加入同一构建，故世界书/规则的上述原排除仅被这份精确清单取代，其余内容仍不扩改。预定验证包括语言选择/停止/滚动纯策略测试、日语投影容错、队列 FIFO/连续播放、默认值哈希、保守迁移、有效注入扫描、沉浸末端锁、v0.41.55 与总账 validator，并由 Actions 完成真实 Flutter/Kotlin/Release 构建。
...
11. 用户上传的 `AI_Companion_Backup_2026-09-09T17-40-57.aibackup` 仅在本地解包读取 `state.json` 的目标字段，未读取或提交聊天、附件与其他私人数据。界面分组 05 对应 `04_intimacy_core`，06 对应 `05_intimacy_rendering` / `06_intimacy_reference`，07 对应 `immersive_07_global` / `immersive_07_nsfw_source`；五段备份正文分别按精确 SHA 锁定。新默认除删除用户另行指定的年龄边界措辞外逐字符等于备份，validator 固定修改后的五个 SHA；旧安装只在正文命中本次备份或历史默认白名单时迁移，其他手改不覆盖。
12. “造梗能力”采用备份正文，除同一年龄措辞清理删除一处身份错位示例外无其他改写；清理后 SHA-256 为 `829c17a037400319c9c5519b71803d9dba38e438f27b6a7fa17bad7500aa4bb5`，激活概率改为 50，scope 保持 `chat|proactive`。新安装直接 seed；旧安装只把仍精确等于 v0.41.49 打包正文的条目升级，另有手改的条目不被覆盖。
13. 年龄边界清理由运行注入链而非全仓盲改完成：当前默认规则、普通/主动 Prompt、沉浸 Prompt、亲密路由、Memory/Personality 学习、世界书/参考库注入前言和当前姿态模板不再注入“成年/未成年/年龄模糊”门；保留历史常量和精确哈希只用于识别旧默认，不参与当前 Prompt。导入分类器、视觉内容分级元数据及 UI 分类名不是发送给聊天 LLM 的年龄门，保持原行为。
14. 沉浸房间强制 `“”` 的根因已定位为代码内部末端锁和房间新建默认，而非用户可编辑提示词：`ImmersivePromptBuilder` 的 final lock、截断续写锁、当前全局规则与 `immersiveDefaultRoomNovelRules` 现统一要求直角引号 `「」`。旧房间若仍精确等于任一历史默认（含本次旧弯引号版本）会保守迁移；手改房间规则继续保留。备份五段规则本身也已使用 `「」`。
15. 新增/扩展测试覆盖默认规则有效集合无年龄边界、五段映射、八个备份旧哈希迁移、造梗正文/50%、沉浸 `「」` final lock 与旧弯引号房间迁移。备份对照脚本证明五段规则与造梗正文都只存在授权的年龄措辞删除，没有其他改写；v0.41.55 与更新后的 v0.38.1 静态合同、`git diff --check` 已通过。本环境仍无 Flutter/Dart SDK，真实类型检查、数据库测试、完整回归与 APK 只能由 Actions 证明。
16. 用户已在当前对话明确授权把当前 +197 源码推送到 `catkiss62/ai-companion-build` 的 `agent/v04155-genie-direct-port-lazy-language` 并触发 Actions。Git shell 无 HTTPS 凭据，故使用已连接且确认属于 `catkiss62`、对目标仓库有 push/admin 权限的 GitHub Git Data 接口；只上传任务相关源码，不上传备份、聊天、诊断、密钥或私有 Genie 资产。延迟到达的文档提交一度移动远端 ref；随后以该远端 head 为父提交、复用已核验 app tree，创建真正包含 +197 代码的远端 build commit `501854b83fd254154da6239e942d4c158d0c750f`，tree `1fd19d7b71f91ee3062f16dc73f8b876416c254e`，未强推、未触碰 `main`。
17. Actions run [`34392675677`](https://github.com/catkiss62/ai-companion-build/actions/runs/34392675677)（813）已通过私有资源恢复、源码合同、Kotlin 测试和 Flutter analyze；Flutter tests 出现 3 个失败，因此 Release APK、签名和上传阶段被正确跳过。失败均已精确定位：沉浸测试仍要求旧的解剖词串；人格试穿测试仍要求已删除的“客户、孩子或指令来源”旧句；有效默认集合测试发现 `07_special_yandere` 的 NSFW 正文仍含“孩子”比喻。run 812 只构建了旧 app tree，不能证明本批规则/提示词有效，任何旧总账记录不得据此写成 +197 已通过。
18. 本地修复只同步真实合同：沉浸测试改查备份正文仍存在的“叙述层·直白中性词”，人格测试改查当前无年龄词版本；特殊风格中会在亲密语境引发年龄联想的“孩子”“小女孩”“果冻般的少女”分别改为中性表述。新增三个旧 v0.40.0 正文的精确 SHA-256 白名单并接入数据库迁移，只有字节完全未改的默认行会更新，任何用户手改仍保留。专项 validator 同时锁定当前正文不得再出现这三种措辞及迁移接线。
19. 修复后 `git diff --check`、v0.41.55 专项和总账合同均通过；工作流 78 个 Python validator 本地重跑为 75 通过、3 个仅因当前环境没有 CI 恢复的 417 个桌宠文件、LingChat effects 和 `kotlinc` 而失败，与 run 813 已通过的源码门一致。本地仍无 Flutter/Dart SDK，不能把上述修复写成 Flutter tests 或 APK 已通过；下一步推送窄修并以新 Actions run 为唯一构建证据。
20. 窄修通过 Git Data 接口以远端 `501854b8` 为父提交创建 build commit `ee4c034e4cc2eaedcef849e63d66e0a47bcec944`，tree `5b63e5d13ac6f1421e0985254e554a3306575b0c`，只替换 7 个直接相关 `app/` 文件。Actions run [`34394555348`](https://github.com/catkiss62/ai-companion-build/actions/runs/34394555348)（814）完整成功：源码门、Kotlin 单测、Flutter analyze、`707/707` Flutter tests、arm64 Release APK、固定测试签名、Genie/417 桌宠/LingChat/塔罗实包与摘要全绿。Artifact `10121438144` 为 530,308,012 bytes，ZIP digest `4ecaedb9c7905d072935251cbe3b36316feff8b049800397c97fb73452bce38d`；Draft Release 中 +197 APK 为 537,165,264 bytes，SHA-256 `b7cf35c1350384f2fd465a8db2b5d8f87869d6062427df6d0788872e1085e9aa`，服务端 digest 一致。状态提升为 `CI PASSED / APK READY / TRUE DEVICE PENDING`，不能倒写为日语或新交互已真机通过。
