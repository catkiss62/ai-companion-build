...
- 冻结归档：`ledger/archive/AI_Companion_总账归档_截至_v0.41.74+218.md`；只读，不参与日常接班与追加更新。
- 后续只修改小型当前入口；达到新的阶段边界时才新建一次只读归档，不把历史重新拼回当前文件。
...
8. 历史过程保留在带截止版本的只读归档；当前记录只写入小型永久入口，并由 validator 同时核对入口容量与归档 SHA-256。
