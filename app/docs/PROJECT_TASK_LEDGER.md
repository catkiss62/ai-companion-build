...
> v0.31.0+40 第一批已落地 Reality Grounding、Thought provenance、纯 Desire policy、action-aware satisfy、fatigue gate、Presence 去重加权与脱敏可观测性，并加入 proactive 最终事实 guard；下列未勾选项继续留在后续 v0.31.x。
...
- [x] 为普通聊天与 proactive generation 增加 Grounding 单元测试，覆盖“你好已回复后用户沉默”“连续两次主动消息”“内部 Thought 文本绝不能被当成 user quote”“当地时间明确注入”等案例。
...
- [x] 脱敏诊断新增 `grounding`：local time/daypart、last user/assistant 时间、pending user turn、last user answered、awareness freshness；不输出聊天正文。
...
### F. 手机 / 平板同一个“她” · Active Brain 真机接管
