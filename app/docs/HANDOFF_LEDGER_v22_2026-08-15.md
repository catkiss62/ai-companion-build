# AI Companion · 接班总账 v22

更新时间：2026-08-15（Asia/Tokyo）

> 本文件继承并取代 `AI_Companion_接班总账_v21_2026-08-15.md`。判断优先级固定为：用户最新明确决定 > GitHub `main` 实际源码与 Actions > 最新脱敏真机诊断 > 仓库 HANDOFF/任务账 > 历史对话与参考资料。讨论、设计、真机线索不得写成已实现功能。

## 0. 本轮结论

- 双通道感官第二方向已经进入 `main`：assistant 回复正文中**实际完成**的触觉动作，以同类用户动作的 0.5 倍形成 `ai_to_self` 短时身体回响。
- detection 是纯计算；assistant message、generation job completed、somatic event 和 aggregate 在同一 SQLite transaction 提交。停止生成、网络/格式失败、stale writer、transfer lock、恢复重跑不会留下“幽灵触感”。
- 事件使用稳定 ID `{assistant_message_id}:ai_to_self:{scene_key}`，并校验事件必须指向当前 assistant message 且 direction 必须为 `ai_to_self`；重复恢复受唯一键和事务共同保护。
- 首版只启用保守触觉词法，覆盖牵手、拥抱、亲吻、抚摸、轻拍、轻捏、揉动、轻蹭、倚靠、轻挠和轻咬；“想、准备、打算、如果、没有、不、不会、不能、差点”等意图/假设/否定不产生自身感觉。
- 版本升级为 `v0.32.1+53`，schema 保持 21。PR #9 已 squash 合并；产品提交 `08f2f46c3e464761a247a7983a3b63233e17dc9f`。
- Actions run #32（ID `31841772104`）通过新旧 validators、Flutter analyze、全部 tests、release APK、Meju A2 原生 payload 字节校验和 artifact 上传。
- 用户决定：电池优化提示后置，目前实测后台未被系统杀死；旧悬浮球不再单独投入大修，后续由桌宠作为入口，但 Android Overlay/WindowManager 生命周期与悬浮聊天窗仍是桌宠必须复用的底层能力。
...
- 默认分支：`main`
- 源码唯一真源：`app/`
- 当前产品代码基线：`08f2f46c3e464761a247a7983a3b63233e17dc9f`。
- 当前 App：`v0.32.1+53 · Somatic AI-to-Self Atomic Echo`
- SQLite schema：`21`
...
10. 旧悬浮球后续由桌宠入口替换；不为旧 UI 做重型翻修，但底层悬浮窗生命周期、触摸、聊天窗和真实停止语义必须保留并复用。
...
- 未实现：smell/taste/sound、Proust 候选、私密 corpus；`ai_to_self` 仍需真机诊断确认。
...
1. 用户安装 v0.32.1+53 后，以她实际回复“（抱住你）/我轻轻牵住你的手”等完成动作获取脱敏诊断，确认 `direction=ai_to_self`、`source=assistant_committed`；“我想抱住你”不得命中。
