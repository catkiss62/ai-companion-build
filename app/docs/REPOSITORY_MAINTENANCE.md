...
- 根目录当前总账是唯一日常入口；冻结归档只读，旧任务不要求每轮重读。
- 当前总账接近 85 KB 或一个大版本阶段结束时，应先创建带截止版本的只读归档，记录 byte count 与 SHA-256，再把当前入口压缩为：永久边界、当前基线、未完成任务、真机待验证和索引。
- 100,000 bytes 是“单个当前入口”的防失控上限，不是项目历史总容量；历史通过多个只读归档和 Git 历史无限延续。
- 归档后必须同步 `validate_current_ledger_handoff.py` 与 `DOCUMENTATION_MAP.md`。不得把旧全文重新拼回当前入口。
