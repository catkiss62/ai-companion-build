# AI Companion · 接班总账 v25

更新时间：2026-08-15（Asia/Tokyo）

> 本文件继承并取代 `AI_Companion_接班总账_v24_2026-08-15.md`。判断优先级固定为：用户最新明确决定 > GitHub `main` 实际源码与 Actions > 最新脱敏真机诊断 > 仓库 HANDOFF/任务账 > 历史对话与参考资料。讨论、设计、真机线索不得写成已实现功能。
...
- `v0.33.1+56` 已由 PR #12 产品 run #47 完整验证；数据库 schema 保持 21。最终 merge SHA 在本节进入 main 后补记。
...
| P2 | ACTIVE/VERIFY | Android 桌宠 D1.1 原动作同构 | 完整素材与 18 动作播放器已通过 run #47；待真机后进入 D2 Overlay |
...
- `DRAGGING` 上游本来就是一张保持帧配合程序性漂浮；退出条件是松手事件，不是自动播完。旧预览看起来“卡住”的主要原因是生命周期缺失，不是缺帧。
...
- 私有仓库保存 151 个校验分片及 `asset_pack.json`；CI 安全恢复脚本验证 ZIP SHA、路径/符号链接、文件数、总字节和 tree hash。分片不进入 APK。

### 15.3 动作协议

- 18 动作：`IDLE BLINK GLANCE THINKING WALKING HAPPY HEAD_PAT TALKING ANGRY POKE_REACT TAIL_REACT EATING SWEEPING SLEEPING DRAGGING FALLING LANDING DIZZY`。
- 中文：待机、眨眨眼、四处看看、发呆、散步、开心、被摸摸、说话、生气、被戳、尾巴被碰、吃东西、扫地、睡觉、抓取中、落下、着陆、眩晕。
- 28 组 asset；210 张 187/238/306px runtime PNG 全保留，201 张由 manifest 直接引用，9 张保留作上游 fallback/runtime 资料。
- Android 直接解析原 manifest；保持尺寸/方向、frame order/duration、`enter/body/exit`、90ms 同素材 crossfade、priority、interruptible、duration、return_state、queued return、程序效果和 throw physics。
- 拖拽移动阈值 6px；松手强制 falling，物理稳定后 landing，重摔再 dizzy。

### 15.4 预览与边界

- 普通 `PetPreviewActivity` 仍是隔离预览，不替换旧悬浮球，也不宣称已完成系统级桌宠。
- 按钮显示中文名与原始 ID；状态栏显示 phase、asset、尺寸档和 frame index；可切左右/背面/正面及 187/238/306px。
- D2 才把同一播放器接入独立 Overlay Pet window，并复用点击聊天、悬浮聊天时间、旧 Overlay 生命周期和回退入口。

### 15.5 GitHub 与验证

- 版本 `v0.33.1+56`；schema 21。分支 `codex/v0331-desktop-pet-source-parity`；PR #12。
- 上游参考：`https://github.com/QCYTSN/ds-local-pet`；审计 HEAD `d78094b2cf04e1cc3be5d14a31cfedd8ed059579`。
- 产品实现 `a3265e794cb700330b92cf88b29efb88845b01b2`；Kotlin 修复 `bab971bf944458d2d2bb4b973fc03c7138104a5e`。
- run #47（`31867409197`）完整通过；artifact `9242561565`，ZIP digest `sha256:4058b67b7d8739c57dae6442306fc6524c81229e6b542d56c15c206e2aeafac9`。
- APK 238,499,224 bytes；SHA-256 `456d618776b1729353ea1735a63a139eb344cab9e1b296066bdbed04ef1759b7`。下载后独立读取 APK 再确认 417 source 文件、210 runtime PNG、1 原 manifest、1 中文标签表。
- 最终 merge SHA：待 PR #12 合并后补记；除此以外产品验证已完成。
