# AI Companion · 接班总账 v32

更新时间：2026-08-16（Asia/Tokyo）

> 本文件继承并取代 `AI_Companion_接班总账_v31_2026-08-16.md`。判断优先级固定为：用户最新明确决定 > GitHub `main` 实际源码与 Actions > 最新脱敏真机诊断 > 仓库 HANDOFF/任务账 > 历史对话与参考资料。讨论、设计、真机线索不得写成已实现功能。

## 0. 本轮结论

- `v0.33.7+62`（桌宠坠落视觉还原 D3.1.2）已完成并进入 `main`：PR #18 squash merge `add924be3845c00025633ff31e39bbd4e6802575`，schema 保持 21，无数据库迁移。
- 用户真机确认 `falling_v2.png` 的悬空观感不如上一版，因此完全撤回 v0.33.6 的两段坠落实验：不再运行时注册候选图，不再增加内部 `BOUNCING`，物理层删除 `floorContact`。
- 空中、普通抛落、反弹与短距离轻放全部恢复 v0.33.5 的单一 `FALLING` 姿势和原时序；短距离为 `FALLING → 约180ms → LANDING`，普通抛落稳定时直接 `LANDING`，重摔仍可接 `DIZZY`。
- 用户撤销 `walk_side_stand` 的后续使用计划：走路停止后突然回到正面姿势更自然也更萌，侧面停顿不进入 D3.2。
- 哈欠计划继续有效：`daily_transition_00 → sleepy_yawn → sleepy_yawn（停顿）→ daily_transition_00`，建议约 1.2 秒，只由疲劳、深夜或准备睡觉的自主意图触发。
- v0.33.6 的其他修正保持：App/悬浮聊天统一驱动 THINKING/TALKING、TTS 合成静默、角标头部草莓位置、三档自适应与竖屏 16dp 底边。
- Actions run #63（ID `31905499926`）全绿；artifact `9252379865`，ZIP digest `sha256:4e389332c56ed0af347a04aa122ea5a52648b9df67697914401640baf97ad4fa`。APK 238,610,384 bytes，SHA-256 `663544a5fb64d0d560b31e1c597d77a241e1ba2e51857a676efe5cba0647fc39`；独立确认 ZIP/APK 无损及 417 文件桌宠 source pack 完整。

- `v0.33.6+61`（桌宠聊天状态收尾 D3.1.1）已完成并进入 `main`：PR #17 squash merge `a813cbfda008d1bdad2c478dbb68667acbda92b8`，schema 保持 21，无数据库迁移。
- App 内聊天与原生悬浮聊天现在共用同一桌宠表达状态：reasoning/等待正文为 `THINKING`，正文流式输出为 `TALKING`；正文提交完成后立即结束 generation cue，不把记忆提取等 post-turn 工作误算为说话。
- TTS `synthesizing` 单独存在时不强制任何动作；只有真实 `playing` 才触发 `TALKING`。若正文仍在流式输出，则继续由正文保持 `TALKING`。
- 未读数字移动到头部右上方草莓位置，三档尺寸按窗口顶部 8%、右侧 21% 自适应；竖屏底边从 10dp 再上抬到 16dp，其他边界不变。
- 坠落动作拆为两段：空中使用素材包已有 `falling_v2.png`；第一次触地、后续反弹及轻放触地短帧使用原 `falling`，再进入 `LANDING`，重摔仍可接 `DIZZY`。
- 动作资产口径确认：`processed/` 是按动作处理好的逐帧 PNG 池，实际注册映射仍以 `assets/manifests/actions.json` 为准；候选目录中没有 GIF 的 PNG 也可以显式注册并逐帧播放，本版即复用了 `falling_v2.png`。
- v0.33.6 当时冻结了侧站与哈欠两项安排；v0.33.7 已撤销 `walk_side_stand`，仅保留哈欠：`daily_transition_00 → sleepy_yawn → sleepy_yawn（停顿）→ daily_transition_00`，建议总长约 1.2 秒，只由疲劳、深夜或睡眠意图触发。
- Actions run #62（ID `31903612421`）全绿；artifact `9251889125`，ZIP digest `sha256:472d381b11b017627beb0eb71d9869acf216fd93d479d17d42bbce8a82a58a96`。APK 238,610,776 bytes，SHA-256 `207adf1ebd433a75000b8dc50c7f67bedaceb42bc1f4ed6f9c2fd688ea0e90a3`；下载后独立确认 ZIP/APK 无损，含完整 417 文件桌宠 source pack 与指定 6 个 TTS/native 库的逐字节校验。
...
- 默认分支：`main`
- 源码唯一真源：`app/`
- 当前已合并基线为 `v0.33.7+62`，main squash 提交 `add924be3845c00025633ff31e39bbd4e6802575`，schema 21；PR #18 已合并。
- 已合并分支：`codex/v0337-pet-falling-visual-rollback-d3-1-2`；PR #18。产品实现 head 为 `8391725db536ced2610e103f198bd2ba17533abf`。
- 当前版本：`v0.33.7+62`；schema 21，不做数据库迁移。
- 本版改动：完全恢复 v0.33.5 的单一 `FALLING` 姿势与原落地时序，撤销 `falling_v2`/`BOUNCING`/`floorContact`；取消 `walk_side_stand` 计划，保留哈欠计划；其他 v0.33.6 聊天/TTS、角标和 16dp 底边改动不变。
- Clean Freeze 继续有效：仓库根目录不使用历史 patch/ZIP 作为构建输入；workflow 只验证和构建已提交的 `app/`。
- PR #18 已 squash 合并；main 提交 `add924be3845c00025633ff31e39bbd4e6802575`。
- 产品 Actions run #63（ID `31905499926`）通过完整素材恢复、全部历史 validators、新 D3.1.2 validator、Kotlin 桌宠单测、Flutter analyze/tests、release APK、native/payload 核验及 artifact 上传。
- 交付 Artifact `9252379865`，名称 `AI-Companion-v0.33.7-62-Pet-Falling-Visual-Rollback-D3-1-2-APK`，ZIP digest `sha256:4e389332c56ed0af347a04aa122ea5a52648b9df67697914401640baf97ad4fa`。
- 交付 APK SHA-256：`663544a5fb64d0d560b31e1c597d77a241e1ba2e51857a676efe5cba0647fc39`；238,610,384 bytes。
...
## 21. v0.33.6+61 · 桌宠聊天状态收尾 D3.1.1

> 本节记录 v0.33.6 当时实现；其中两段坠落和 `walk_side_stand` 安排已被 v0.33.7（第 22 节）撤销，哈欠安排继续有效。

### 21.1 用户决定与动作资产口径

- TTS 合成阶段不做 `TALKING`；可以保持当前非对话动作或待机，不为了“正在合成”伪造说话。正文流式输出和真实 TTS 播放仍可使用 `TALKING`。
- `processed/` 保存已经处理好的逐帧 PNG 资产，但“哪些是注册动作、动作如何组合”以 `assets/manifests/actions.json` 为准。候选目录里只有 PNG、没有 GIF 的素材也可由播放器逐帧组合；不能仅凭有没有 GIF 判断动作是否可用。
- `walk_side_stand` 三张图用于后续散步暂停、转向或走到边缘的过渡，不替换普通正面待机。
- `falling_v2.png` 用于空中；旧 `falling` 用于第一次接触地面、反弹几下和轻放触地的短帧，然后进入 `LANDING`。
- `sleepy_yawn.png` 留到 D3.2：`daily_transition_00 → sleepy_yawn → sleepy_yawn（停顿）→ daily_transition_00`，建议约 1.2 秒，其中哈欠图停 0.5～0.7 秒；只由疲劳、深夜或准备睡觉的自主意图触发。

### 21.2 对话与 TTS 状态

| 真实状态 | 桌宠 cue / 动作 |
|---|---|
| generation active，尚无正文 | `THINKING` |
| generation active，正文流式输出 | `TALKING` |
| generation 已完成，TTS synthesizing | 不强制动作（idle cue） |
| TTS playing | `TALKING` |
| 无 generation、无播放 | 不强制对话动作 |

- App 内 `ChatController` 将 generation/TTS 状态通过现有 Android bridge 送入同一个 `OverlayBubbleService`；原生悬浮聊天与 App 聊天不再是两套桌宠表现源。
- assistant 正文提交后立即把 generation cue 置 idle，再开始非流式 TTS 合成或 post-turn 工作，因此合成空档不会保持 `TALKING`。
- 手动朗读、自动朗读和流式 TTS 的 `playing` 都能触发 `TALKING`；只有实际播放生效。
- 触碰/物理瞬态动作的优先级与 v0.33.5 相同，结束后若对话 cue 仍有效才恢复 THINKING/TALKING。

### 21.3 几何与两段坠落

- 竖屏底部安全距离从 10dp 改为 16dp；竖屏上/左/右和全部横屏范围不变。
- 未读角标为 22dp，锚点按桌宠窗口尺寸计算：顶部 8%、右侧 21%；小/中/大三档调整大小时即时重算。
- manifest 载入时保留对上游 18 个注册行为动作的完整校验，再增加内部 `falling_airborne_v2` asset 与内部 `BOUNCING` action；没有修改、删除或重排上游动作真源。
- `FALLING` 的 body 改读候选包现有单帧 `falling_v2.png`；`BOUNCING` 复用原 `falling` asset。
- 物理层新增 `floorContact` 瞬时信号：首次触地且仍在反弹时切 `BOUNCING`；稳定落地时至少保留约 180ms 的原 `falling` 触地帧，再进 `LANDING`；重摔随后可排队 `DIZZY`。

### 21.4 代码、测试与验证

- 新增 `app/docs/PET_CHAT_STATE_FINALIZATION_D3_1_1_v0.33.6.md` 与 `app/tools/validate_v0336_pet_chat_state_finalization.py`。
- Kotlin 单测新增：TTS 仅 synthesizing 且 generation inactive 必须为 idle；抛掷物理必须报告至少一次 floor contact。
- 全部历史 validators、新 validator、Kotlin 桌宠单测、Flutter analyze、126 个 Flutter tests、release APK、417 文件素材载荷和指定 6 个 arm64 TTS/native 库逐字节核验均通过。
- 本版没有数据库变更，schema 继续为 21。

### 21.5 GitHub、Actions 与交付证据

- 分支：`codex/v0336-pet-chat-state-finalization-d3-1-1`
- PR #17：`https://github.com/catkiss62/ai-companion-build/pull/17`
- 产品 head：`1083760c9b7e71e1fb161bc80800f7920c998f0f`
- main squash：`a813cbfda008d1bdad2c478dbb68667acbda92b8`
- 产品 run #62：`31903612421`，结论 success
- artifact：`9251889125`；名称 `AI-Companion-v0.33.6-61-Pet-Chat-State-Finalization-D3-1-1-APK`
- artifact ZIP 大小：238,611,232 bytes；digest：`sha256:472d381b11b017627beb0eb71d9869acf216fd93d479d17d42bbce8a82a58a96`
- APK：238,610,776 bytes；SHA-256 `207adf1ebd433a75000b8dc50c7f67bedaceb42bc1f4ed6f9c2fd688ea0e90a3`
- 当前状态：`IMPLEMENTED / AUTOMATION PASS / REAL-DEVICE VERIFY`。真机重点验证 App 内 reasoning/正文/手动与自动语音、正文结束到语音开始的静默空档、三档未读位置、16dp 竖屏底边，以及甩出后的空中→触地/反弹→落地切换。v0.33.6 当时锁定了 `walk_side_stand` 与哈欠序列；前者已在 v0.33.7 撤销，后者继续保留，二者均不得写成已实现。


## 22. v0.33.7+62 · 桌宠坠落视觉还原 D3.1.2

### 22.1 用户真机结论

- 候选素材没有被原 manifest 注册是有视觉原因的：`falling_v2.png` 在真机上的悬空效果不如 v0.33.5 的原姿势。
- 本轮不是继续调两段坠落，而是完全还原上一版的悬空细节，包括短距离轻放、普通抛落、反弹和稳定落地时序。
- `walk_side_stand` 不再使用。市面桌宠走停后突然切回正面姿势更常见，也更有可爱感；侧面停顿可能显得突兀。
- 哈欠动作仍有明确用途，继续保留给 D3.2。

### 22.2 精确还原内容

| 路径 | v0.33.7 当前行为 |
|---|---|
| 空中/抛落 | 使用上游原注册 `FALLING` |
| 发生反弹 | 继续原 `FALLING`，不切内部 `BOUNCING` |
| 普通抛落稳定 | 按 v0.33.5 直接进入 `LANDING` |
| 低高度轻放 | `FALLING → 约180ms → LANDING` |
| 重摔 | `LANDING` 后仍可排队 `DIZZY` |

- `PetSkinManifest.kt` 删除 v0.33.6 的 `falling_airborne_v2` asset 注入和内部 `BOUNCING` action，恢复只校验/使用上游 18 个注册行为动作。
- `PetThrowPhysics.kt` 删除额外 `floorContact` 字段和信号，恢复 v0.33.5 数据结构。
- `PetOverlayWindow.kt` 删除首次触地切 `BOUNCING` 和稳定前 180ms 触地补帧；`playLightLanding` 恢复先播放原 `FALLING`。
- `PetSkinManifest.kt`、`PetThrowPhysics.kt`、`PetActionStateMachineTest.kt` 的 Git blob SHA 分别恢复为 v0.33.5 的 `fccd143d382a80659eae80b6167781314f6b6bb1`、`8b86d50737ba17a440c7d2c10c0f5d7a1b01eca5`、`7fd60ec1ee9ca2120b08c250450bdc1e49970f3c`。

### 22.3 后续动作决定

- 明确不使用 `walk_side_stand`，不得在 D3.2 的散步暂停、转向或边缘停顿中重新引入。
- 哈欠保留：`daily_transition_00 → sleepy_yawn → sleepy_yawn（停顿）→ daily_transition_00`。
- 哈欠建议总长约 1.2 秒，其中 `sleepy_yawn` 停 0.5～0.7 秒；只由疲劳、深夜或准备睡觉的自主意图触发，不用于 TTS 合成等待。
- `falling_v2.png` 和 `walk_side_stand` 文件仍随完整 source pack 保留，方便审计，不代表运行时注册或未来计划。

### 22.4 保持不变

- App 内聊天和原生悬浮聊天继续共用 THINKING/TALKING 状态。
- TTS `synthesizing` 不触发 TALKING；正文流式输出和真实 `playing` 才触发。
- 22dp 未读角标继续按窗口顶部 8%、右侧 21% 自适应三档尺寸。
- 竖屏底部安全距离继续为 16dp，其他边界不变。
- schema 继续为 21，无数据库迁移。

### 22.5 GitHub、Actions 与交付证据

- 分支：`codex/v0337-pet-falling-visual-rollback-d3-1-2`
- PR #18：`https://github.com/catkiss62/ai-companion-build/pull/18`
- 产品 head：`8391725db536ced2610e103f198bd2ba17533abf`
- main squash：`add924be3845c00025633ff31e39bbd4e6802575`
- 产品 run #63：`31905499926`，结论 success
- artifact：`9252379865`；名称 `AI-Companion-v0.33.7-62-Pet-Falling-Visual-Rollback-D3-1-2-APK`
- artifact ZIP 大小：238,610,840 bytes；digest：`sha256:4e389332c56ed0af347a04aa122ea5a52648b9df67697914401640baf97ad4fa`
- APK：238,610,384 bytes；SHA-256 `663544a5fb64d0d560b31e1c597d77a241e1ba2e51857a676efe5cba0647fc39`
- 自动验证：全部历史 validators、新 D3.1.2 精确回退 validator、Kotlin 桌宠单测、Flutter analyze、126 个 Flutter tests、release APK、417 文件 source pack 与指定 6 个 arm64 TTS/native 库逐字节核验全部通过。
- 当前状态：`IMPLEMENTED / AUTOMATION PASS / REAL-DEVICE VERIFY`。真机只需重点确认悬空、反弹、低高度轻放和普通落地已经恢复上一版观感；哈欠仍为后续设计，侧站明确不用。
