# AI Companion · 接班总账 v31

更新时间：2026-08-16（Asia/Tokyo）

> 本文件继承并取代 `AI_Companion_接班总账_v30_2026-08-16.md`。判断优先级固定为：用户最新明确决定 > GitHub `main` 实际源码与 Actions > 最新脱敏真机诊断 > 仓库 HANDOFF/任务账 > 历史对话与参考资料。讨论、设计、真机线索不得写成已实现功能。
...
- D3.2 已冻结两项安排但尚未实现：`walk_side_stand` 用于走路暂停/转向；哈欠使用 `daily_transition_00 → sleepy_yawn → sleepy_yawn（停顿）→ daily_transition_00`，建议总长约 1.2 秒，只由疲劳、深夜或睡眠意图触发。
...
- 默认分支：`main`
- 源码唯一真源：`app/`
- 当前已合并基线为 `v0.33.6+61`，main squash 提交 `a813cbfda008d1bdad2c478dbb68667acbda92b8`，schema 21；PR #17 已合并。
- 已合并分支：`codex/v0336-pet-chat-state-finalization-d3-1-1`；PR #17。产品实现 head 为 `1083760c9b7e71e1fb161bc80800f7920c998f0f`。
- 当前版本：`v0.33.6+61`；schema 21，不做数据库迁移。
- 本版改动：App/悬浮聊天统一驱动桌宠对话动作；TTS 合成静默、真实播放说话；角标移到头部草莓位置并按尺寸适配；竖屏底边总计 16dp；空中 `falling_v2` 与触地/反弹原 `falling` 两段化；记录 D3.2 的侧站和哈欠安排。
- Clean Freeze 继续有效：仓库根目录不使用历史 patch/ZIP 作为构建输入；workflow 只验证和构建已提交的 `app/`。
- PR #17 已 squash 合并；main 提交 `a813cbfda008d1bdad2c478dbb68667acbda92b8`。
- 产品 Actions run #62（ID `31903612421`）通过完整素材恢复、全部历史 validators、新 D3.1.1 validator、Kotlin 桌宠单测、Flutter analyze/tests、release APK、native/payload 核验及 artifact 上传。
- 交付 Artifact `9251889125`，名称 `AI-Companion-v0.33.6-61-Pet-Chat-State-Finalization-D3-1-1-APK`，ZIP digest `sha256:472d381b11b017627beb0eb71d9869acf216fd93d479d17d42bbce8a82a58a96`。
- 交付 APK SHA-256：`207adf1ebd433a75000b8dc50c7f67bedaceb42bc1f4ed6f9c2fd688ea0e90a3`；238,610,776 bytes。
...
- 当前状态：`IMPLEMENTED / AUTOMATION PASS / REAL-DEVICE VERIFY`。真机重点验证 App 内 reasoning/正文/手动与自动语音、正文结束到语音开始的静默空档、三档未读位置、16dp 竖屏底边，以及甩出后的空中→触地/反弹→落地切换。D3.2 的 `walk_side_stand` 与哈欠序列仍是已锁定设计，不得写成已实现。
